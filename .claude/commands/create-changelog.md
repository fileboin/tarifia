...
```bash
docker exec tarifia-dev-<N>-api-1 bash -c "cd /app/server && uv run python -m scripts.seeds_load"
```
...
- Log in at `http://localhost:<web-port>/auth` as `admin@tarifia.sh`. The email domain must be real (tarifia.sh works) or login validation rejects it. Grab the login code with `docker logs tarifia-dev-<N>-api-1 --since 30s 2>&1 | grep -oE 'LOGIN CODE: [A-Z0-9]+' | tail -1`.
- If a feature lives in an org that `admin@tarifia.sh` is not a member of, rename the seeded owner's email to a real `@tarifia.sh` domain (e.g. `UPDATE users SET email='merchant@tarifia.sh' WHERE email='support@meltedsql.com'`) and restart the web container so Next.js re-fetches the user's org list: `docker restart tarifia-dev-<N>-web-1`.
