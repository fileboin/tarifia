...
Events are sent to Tarifia using the [Events Ingestion API](/api-reference/events/ingest) and are stored in our database. An event consists of the following fields:
...
- A `customer_id` or `external_customer_id`, which is Tarifia's customer ID or your user's ID. This is used to identify the customer that triggered the event.
