---
title: "Products"
description: "Start selling digital products on Tarifia in minutes."
---

<Info>
**Everything is a product**

Subscriptions and one-time purchases are both products in Tarifia — same API, same data model, just different pricing and billing logic. They live together in the Products dashboard, filterable by pricing model.
</Info>
...
Tarifia supports several pricing models, and you pick one per product:
...
Whether the amount you enter includes tax or has tax added on top depends on your tax behavior setting. By default, Tarifia picks the convention that matches the customer's country (inclusive in most of the world, exclusive in the US, Canada, and India). You can set your own default under **Settings**. See [Tax Inclusive Pricing](/features/tax-inclusive-pricing) for how the amount you set translates to what customers actually pay.
...
Tarifia picks the currency based on the customer's geolocation at checkout. If their currency isn't enabled on the product, it falls back to your organization's default.

<Warning>
**Creating checkout sessions from a backend or proxy?** Tarifia reads the customer's geolocation from the IP address of the request that creates the session. If you create sessions server-side (an API, a Cloudflare Worker, etc.), Tarifia sees *your server's* IP and may pick the wrong currency.
...
Tarifia supports 130+ currencies for product pricing:
...
For recurring products, toggle **Enable trial period** to give customers a window where they aren't charged. Pick a number and a unit (days, weeks, months, or years) and Tarifia handles the rest.

[Learn more about trials](/features/subscriptions/trials).

## Metadata

You can attach arbitrary key–value metadata to a product. It's not shown to customers, but it travels along on every order, subscription, and webhook tied to the product, which makes it useful for keeping track of internal IDs or categories that live outside Tarifia.
...
Tarifia grants and revokes benefits automatically as customers purchase, renew, or cancel. [Learn more about benefits](/features/benefits/introduction).
...
<AccordionGroup>
  <Accordion title="What about variants? Can I sell a monthly and a yearly version of the same product?">
    Tarifia takes a different approach to what the industry typically calls **variants**. Each product has a single pricing model, and instead of bolting variants onto one product, you create one product per pricing model and showcase them together at checkout.
  ...
  <Accordion title="Will customers see prices in their local currency?">
    If you've enabled additional payment currencies on the product, yes — Tarifia matches the customer's geolocation to one of the enabled currencies. If there's no match, it falls back to your organization's default. See [Multiple payment currencies](#multiple-payment-currencies) above.
  </Accordion>
