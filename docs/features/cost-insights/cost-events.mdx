...
Cost Insights works by allowing you to add a special `_cost` property to any event you ingest through Tarifia's Event Ingestion API. These costs are then aggregated and made available through the Metrics API alongside revenue metrics.
...
```json cURL
POST https://api.tarifia.sh/v1/events/ingest
