---
title: "Orders"
description: "Every paid transaction on Tarifia is an order."
---

An **order** is the record of a single paid transaction on Tarifia. Every successful checkout, subscription cycle, and subscription change generates an order — it's where the money, the tax, the invoice, and the link back to the [product](/features/products) and [customer](/features/customer-management) live.
...
Tarifia creates an order in all of these cases:
...
Sometimes you need to charge a customer an amount that doesn't originate from a checkout or a subscription renewal — a usage overage, a one-off professional-services fee, or a manual top-up. Tarifia lets you run these **off-session charges** against a customer's saved payment method, without the customer being present.
...
The customer must have a complete billing address (so Tarifia can calculate tax) and at least one saved payment method.

```bash cURL
curl --request POST \
  --url https://api.tarifia.sh/v1/orders/ \
...
```bash cURL
curl --request POST \
  --url https://api.tarifia.sh/v1/orders/<order_id>/finalize \
  --header 'Authorization: Bearer <YOUR_BEARER_TOKEN_HERE>' \
  --header 'Content-Type: application/json' \
  --data '{}'
```
...
| Status               | Meaning                                                                                  |
| -------------------- | ---------------------------------------------------------------------------------------- |
| `pending`            | The order has been created and Tarifia is attempting to collect payment.                   |
...
- **Tax details**: taxability reason, tax rate, and the amount collected — useful if you're operating as merchant of record on your own, or reconciling against Tarifia's [MoR tax handling](/merchant-of-record/introduction).
...
Tarifia generates a PDF invoice for every paid order. You can:
...
A **receipt** is the proof of payment for an order — what was charged, how, and what's been refunded since. Tarifia issues one for every paid order and assigns a per-customer number in the form `RCPT-{customer-id}-{NNNN}`.
...
If you're integrating orders into your own system, Tarifia emits an event on every state transition:
