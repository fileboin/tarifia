...
You can easily sell software license keys with Tarifia without having to deal with sales tax or hosting an API to validate them in real-time. License keys with Tarifia come with a lot of powerful features built-in.

* Brandable prefixes, e.g `TARIFIA_*****`
...
1. Go to [`Benefits`](https://tarifia.sh/to/dashboard/products/benefits)
...
**Enable user to deactivate instances via Tarifia.** Instead of building your own custom admin for customers to manage their activation instances - leave it to Tarifia instead.
...
It's super easy and straightforward to integrate Tarifia license keys into your application, library or API.
...
```bash Terminal
curl -X POST https://api.tarifia.sh/v1/customer-portal/license-keys/activate
...
```bash Terminal
curl -X POST https://api.tarifia.sh/v1/customer-portal/license-keys/validate
-H "Content-Type: application/json"
-d '{
  "key": "1C285B2D-6CE6-4BC7-B8BE-ADB6A7E304DA",
  "organization_id": "fda84e25-7b55-4d67-916d-60ead04ff61f",
