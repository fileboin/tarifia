...
There are three ways your customers can land on the Customer Portal: the default URL, a pre-authenticated link you generate from your application, or the links Tarifia automatically includes in the emails it sends to your customers.
...
```
https://tarifia.sh/<your-org-slug>/portal
```

Customers authenticate by entering the email address they used to purchase or subscribe. Tarifia then emails them a one-time code to complete sign-in.
...
async function run() {
  const result = await tarifia.customerSessions.create({
    customerId: "<value>",
  });
...
If you're on Next.js, the `@tarifia-sh/nextjs` adapter wraps this into a single route handler:
...
export const GET = CustomerPortal({
  ...
  server: "sandbox", // Use sandbox if you're testing Tarifia — pass 'production' otherwise
});
```
...
## 3. Emails Tarifia sends to your customers

You don't have to build anything to get customers to the portal — Tarifia already includes a link to it in the transactional emails we send on your behalf, including:
