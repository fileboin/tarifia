...
with Tarifia(
    ...
    checkout = tarifia.checkouts.create(request={
        "products": ["<product_id>"],
        "prices": {
            "<product_id>": [
                {
...
Quite often, you'll have your own users management system in your application, where your customer already have an ID. To ease reconciliation between Tarifia and your system, you can inform us about your customer ID when creating a checkout session through the [`external_customer_id`](/api-reference/checkouts/create-session/) field.

After a successful checkout, we'll create a Customer on Tarifia with the external ID you provided. It'll be provided through the `customer.external_id` property in webhooks you may have configured.
...
When you create a checkout session, Tarifia uses the IP address of the request to detect the customer's country. This drives features like:
...
If you use [checkout links](/features/checkout/links), this works automatically. But if you create sessions through the API from a **backend, proxy, or edge function** (e.g. your own API, a Cloudflare Worker, a Next.js route handler), Tarifia will see *your server's* IP — not the customer's — and the detection will be wrong.
...
<Note>
  When `customer_ip_address` is provided, you don't need to set `customer_billing_address.country` yourself — Tarifia will derive both the country and the currency from the IP.
</Note>
