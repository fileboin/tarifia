...
<img src="https://tarifia.sh/docs/assets/features/checkout/embed/demo.png" />
...
<a
  ...
  data-tarifia-checkout
  data-tarifia-checkout-theme="light"
>
  Purchase
</a>
...
You can style the trigger element any way you want, as long as you keep the `data-tarifia-checkout` attribute.
...
Then, you should import the `TarifiaEmbedCheckout` helper class and manually call `TarifiaEmbedCheckout.init()`. This will add the required handlers on elements having the `data-tarifia-checkout` attribute.
...
For users who need more control over the embedded checkout flow, the `TarifiaEmbedCheckout` class provides several advanced features.

### Programmatically creating an embed

Instead of using declarative triggers with `data-tarifia-checkout` attributes, you can programmatically create and control checkout instances:
...
const openCheckout = async () => {
  ...
  try {
    ...
    const checkout = await TarifiaEmbedCheckout.create(checkoutLink, {
      theme,
    });
...
const openCheckoutWithEvents = async () => {
  const checkout = await TarifiaEmbedCheckout.create("__CHECKOUT_LINK__", {
    onLoaded: (event) => {
...
const CheckoutButton = () => {
  ...
  const handleCheckout = async () => {
      try {
        const checkout = await TarifiaEmbedCheckout.create(
          '__CHECKOUT_LINK__',
          {
            theme: 'light',
            onLoaded: (event) => {
...
To enable wallet payment methods on your embedded checkout, please [email us](mailto:support@tarifia.sh) with:
