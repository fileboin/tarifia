...
When displaying a price to a customer, there are two common conventions: either the displayed price **already includes tax** (inclusive), or tax is **calculated and added on top** at checkout (exclusive). Tarifia gives you fine-grained control over this behavior at both the organization level and on individual product prices.
...
| Option             | Description                                                                                        |
| ------------------ | -------------------------------------------------------------------------------------------------- |
| **Location-based** | Tarifia automatically picks the right behavior based on the customer's country. This is the default. |
...
When the tax behavior is set to **Location-based**, Tarifia resolves the actual inclusive/exclusive behavior at checkout time based on the customer's billing address:
...
Regardless of the tax behavior chosen, Tarifia always:
