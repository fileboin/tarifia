...
Tarifia ships with a built-in analytics dashboard so you can stay focused on growing the business rather than wiring up reporting.
...
When you pick a date range, Tarifia splits it into equal-width **intervals** — one hour, one day, one week, one month, or one year each — and aggregates every metric inside each interval. Each interval (sometimes called a "bucket") is one point on the chart and one row in the API response.
...
- **Revenue** — Gross revenue from every completed order in the interval: one-time purchases, new subscription orders, and renewal orders. Computed on the order subtotal, before Tarifia fees and tax.
...
- **Net Revenue** — Revenue after deducting Tarifia's platform fee and the payment processor's fee on each order. This is the amount actually paid out to you.
