...
Whenever a subscription's price changes mid-cycle — typically because the customer switched to a different product or changed seats — there's an unused portion of the current billing period that has already been paid for. **Proration** is how Tarifia reconciles that difference.

You pick the proration behavior either at the organization level (as the default) or per API call.

## Proration behaviors

Tarifia supports three standard proration behaviors:

### Invoice Immediately (`invoice`)

The subscription is updated **immediately** and Tarifia invoices the prorated difference right away. If the update is an upgrade the customer is charged; if it's a downgrade they're credited on the new invoice.
...
```bash cURL
curl --request PATCH \
  --url https://api.tarifia.sh/v1/subscriptions/{subscription_id} \
...
with Tarifia(access_token="<YOUR_BEARER_TOKEN_HERE>") as tarifia:
    res = tarifia.subscriptions.update(
        id="<subscription_id>",
        subscription_update={
            "product_id": "<product_id>",
...
Tarifia prorates on a **per-second** basis. If `S` seconds remain out of `T` total seconds in the current billing period:
...
<Note>
  ...
  matters: upgrading at noon credits a different amount than upgrading at midnight. Tarifia always
