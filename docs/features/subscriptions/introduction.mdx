...
1. **A customer subscribes.** When a customer completes a [checkout](/features/checkout/session) for a product that has a recurring price, Tarifia creates a `subscription` and the first [order](/features/orders).
2. **Tarifia renews it automatically.** At the end of each billing period, Tarifia advances the subscription to the next cycle and creates a new order, then attempts to charge the customer's default payment method. If the payment fails, the subscription moves to `past_due` and Tarifia runs an automated [payment recovery flow](/features/subscriptions/failed-payments).
...
You can turn renewal reminders off under [**Settings → Customer notifications**](https://tarifia.sh/to/dashboard/settings) if you prefer to handle that communication yourself.
...
- **Through the API, for free recurring products only.** You can call [Create Subscription](/api-reference/subscriptions/create) to subscribe an existing customer to a free product — no order, no email, no charge. This is useful for freemium onboarding flows where you want every signup to have a subscription tied to it from day one. Paid products always go through checkout so that Tarifia can collect and validate the payment method.
...
<CardGroup cols={2}>
  ...
  >
    How Tarifia retries failed renewals and when benefits are revoked.
  </Card>
  <Card
...
<AccordionGroup>
  <Accordion title="What's the difference between a subscription and a product?">
    ...
    Tarifia treats one-time purchases and recurring subscriptions as the same
  ...
  <Accordion title="When is a customer actually charged?">
    At the end of each billing cycle. When a subscription renews, Tarifia
    ...
    charge fails the subscription moves to `past_due` and Tarifia starts the
  ...
  <Accordion title="What happens to benefits when a subscription is canceled?">
    ...
    - If a renewal payment can't be recovered after Tarifia's retries, the
