...
```
pnpm add @tarifia-sh/ingestion
```

```typescript
import { Ingestion } from '@tarifia-sh/ingestion';
import { StreamStrategy } from '@tarifia-sh/ingestion/strategies/Stream';

const myReadstream = createReadStream(...);

// Setup the Stream Ingestion Strategy
const streamIngestion = Ingestion({ accessToken: process.env.TARIFIA_ACCESS_TOKEN })
