...
```
pnpm add @tarifia-sh/ingestion @aws-sdk/client-s3
```

```typescript
import { Ingestion } from "@tarifia-sh/ingestion";
import { S3Strategy } from "@tarifia-sh/ingestion/strategies/S3";
...
const s3Ingestion = Ingestion({ accessToken: process.env.TARIFIA_ACCESS_TOKEN })
...
export async function POST(request: Request) {
  try {
    ...
    const s3 = s3Ingestion.client({
      customerId: request.headers.get("X-Tarifia-Customer-Id") ?? "",
    });
