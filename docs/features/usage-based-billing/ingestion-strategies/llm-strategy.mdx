...
```
pnpm add @tarifia-sh/ingestion ai @ai-sdk/openai
```

```typescript
import { Ingestion } from "@tarifia-sh/ingestion";
import { LLMStrategy } from "@tarifia-sh/ingestion/strategies/LLM";
import { generateText } from "ai";
import { openai } from "@ai-sdk/openai";

// Setup the LLM Ingestion Strategy
const llmIngestion = Ingestion({ accessToken: process.env.TARIFIA_ACCESS_TOKEN })
...
Our Python SDK includes an ingestion helper and strategies for common use cases. It's installed as part of the Tarifia SDK.
...
The ingestion helper is a simple wrapper around the Tarifia events ingestion API. It takes care of batching and sending events to Tarifia in the background, without blocking your main thread.

```python
import os
from tarifia_sdk.ingestion import Ingestion

ingestion = Ingestion(os.getenv("TARIFIA_ACCESS_TOKEN"))
...
With our PydanticAI strategy, you can easily track the usage of LLMs and send the data to Tarifia for billing.

```python
import os
from tarifia_sdk.ingestion import Ingestion
from tarifia_sdk.ingestion.strategies import PydanticAIStrategy
