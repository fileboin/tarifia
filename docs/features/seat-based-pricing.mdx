...
<Steps>
  <Step title="Create a new product">
    From your dashboard, [create a new product](https://tarifia.sh/to/dashboard/products/new).
  </Step>
...
<Info>
When using the default Tarifia confirmation page (no custom `success_url`), the buyer's seat is automatically claimed during checkout, granting them immediate access to benefits. Any remaining seats can be assigned to teammates.
...
- **Assign seats** by email, external customer ID, or existing Tarifia customer ID
