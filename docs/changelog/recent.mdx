...
description: Stay up to date with the latest changes and improvements to Tarifia.
---
...
* Member sessions (`tarifia_mst_`) preserve identity and permissions
...
* Fixed Tarifia logo not linking to home
...
* Updated Tarifia logo in all emails
...
New dedicated downloads page at tarifia.sh/downloads for easy access to desktop and mobile apps.
...
Tarifia mobile app added to the monorepo:
...
<Update label="2025-10-17">
  ...
  Merchants can now [update existing subscriptions](https://tarifia.sh/docs/api-reference/subscriptions/update#subscriptionupdateproduct) from archived pricing schemes to current ones within the same product.
  ...
  From **October 27th, 2025**, [new IP ranges](https://tarifia.sh/docs/integrate/webhooks/delivery#ip-allowlist) will be added.

  ## Improved Subscription Cancellation Flow

  [Benefits](https://tarifia.sh/docs/features/benefits/introduction) attached to the subscription [are now automatically revoked](https://github.com/tarifiasource/tarifia/pull/7271) when the subscription is canceled.

  ## Ability to specify External ID

  [External ID can now be specified](https://github.com/tarifiasource/tarifia/pull/7275) during creation of a customer via the dashboard.
  ...
  A Return URL can now be set while generating a [checkout session](https://tarifia.sh/docs/api-reference/checkouts/create-session#body-return-url-one-of-0) or a [customer portal session](https://tarifia.sh/docs/api-reference/customer-portal/sessions/create#body-one-of-0-return-url-one-of-0). This allows you to preserve the context for the end users who visit either if they wish to go back to the application.

  ## Ability to search Customers via their External ID

  The [List Customers API](https://tarifia.sh/docs/api-reference/customers/list) now accepts Customer's External ID [in the **query** parameter](https://tarifia.sh/docs/api-reference/customers/list#parameter-one-of-0).
...
We're excited to introduce a chat widget to the Tarifia dashboard, making it easier than ever for merchants to get help directly within the product.
...
We've updated our webhooks server implementation to [include a timestamp in each payload](https://github.com/tarifiasource/tarifia/pull/6770), in line with the Standard Webhooks specification.
...
Customer emails now appear to come from your organization (e.g., "YourOrg (via Tarifia)") with replies directed to your organization's email address, providing a more professional and branded experience for your customers.
...
We're excited to announce the official launch of our [Zapier integration](https://zapier.com/apps/tarifia/integrations)! Get started now and connect Tarifia to 2,000+ other web services.
...
thrilled to have shipped a Tarifia plugin for Better Auth - in collaboration with them.
...
We've added support for an `external_id` field on Customers. We believe this will greatly simplify the reconciliation process between your system and Tarifia.
...
With `external_id`, you can now fetch a Customer directly by their external ID through dedicated `GET`, `PATCH`, and `DELETE` endpoints. You don't even need to store Tarifia's internal ID in your system anymore! [Read more](/features/customer-management#external-id)
...
<Update label="2025-02-19">
  ## Tarifia's take on Product variants
