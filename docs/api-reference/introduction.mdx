...
<CardGroup cols={2}>
  <Card title="Production Base URL" icon="globe">
    `https://api.tarifia.sh/v1`
  </Card>
  <Card title="Sandbox Base URL" icon="flask">
    `https://sandbox-api.tarifia.sh/v1`
  </Card>
...
| ----------- | --------------------------------- | ------------------------------- |
| Production  | `https://api.tarifia.sh/v1`         | Real customers & live payments  |
| Sandbox     | `https://sandbox-api.tarifia.sh/v1` | Safe testing & integration work |
...
```http
Authorization: Bearer tarifia_oat_xxxxxxxxxxxxxxxxx
```
...
curl https://api.tarifia.sh/v1/products/ \
  -H "Authorization: Bearer $TARIFIA_OAT" \
  -H "Accept: application/json"
```

```bash curl (Sandbox - Core API)
curl https://sandbox-api.tarifia.sh/v1/products/ \
  -H "Authorization: Bearer $TARIFIA_OAT_SANDBOX" \
  -H "Accept: application/json"
```

```bash curl (Customer Portal API)
curl https://api.tarifia.sh/v1/customer-portal/orders/ \
  -H "Authorization: Bearer $TARIFIA_CUSTOMER_TOKEN" \
  -H "Accept: application/json"
```
...
```py Python
from tarifia import Tarifia

client = Tarifia(
    access_token=os.environ["TARIFIA_ACCESS_TOKEN"],
    server="sandbox",
)
````
...
```go Go
s := tarifiago.New(
  tarifiago.WithServer("sandbox"),
  tarifiago.WithSecurity(os.Getenv("TARIFIA_ACCESS_TOKEN")),
)
```

```php PHP
$sdk = Tarifia\Tarifia::builder()
    ->setServer('sandbox')
    ->setSecurity(getenv('TARIFIA_ACCESS_TOKEN'))
    ->build();
```
...
List endpoints in the Tarifia API support pagination to help you efficiently retrieve large datasets. Use the `page` and `limit` query parameters to control pagination.
...
<CodeGroup>
```bash Request
curl https://api.tarifia.sh/v1/products/?page=1&limit=100 \
  -H "Authorization: Bearer $TARIFIA_OAT" \
  -H "Accept: application/json"
```
...
Tarifia API has rate limits to ensure fair usage and maintain performance. Limits differ between the **Sandbox** and **Production** environments.
