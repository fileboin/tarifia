---
title: "Integrate Tarifia with Laravel"
sidebarTitle: "Laravel"
description: "In this guide, we'll show you how to integrate Tarifia with Laravel."
---
...
Tarifia Laravel Example App
--------------------------------

We've created a simple example Laravel application that you can use as a reference.

[View Code on GitHub](https://github.com/tarifiasource/tarifia-laravel)
...
To authenticate with Tarifia, you need to create an access token, and supply it to Laravel using a `TARIFIA_API_KEY` environment variable.

You can create an organization access token from your organization settings.

Fetching Tarifia Products for display
------------------------------------------
...
class ProductsController extends Controller
{
    ...
    {
        // Change from sandbox-api.tarifia.sh -> api.tarifia.sh when ready to go live
        // And don't forget to update the .env file with the correct TARIFIA_ORGANIZATION_ID and TARIFIA_WEBHOOK_SECRET
        $data = Http::get('https://sandbox-api.tarifia.sh/v1/products', [
            'is_archived' => false,
        ]);
...
This endpoint will be responsible for creating a new checkout session, redirecting the user to the Tarifia Checkout page & redirect back to a configured confirmation page.
...
class CheckoutController extends Controller
{
    public function handle(Request $request)
    {
        ...
        // Tarifia will replace {CHECKOUT_ID} with the actual checkout ID upon a confirmed checkout
        $confirmationUrl = $request->getSchemeAndHttpHost() . '/confirmation?checkout_id={CHECKOUT_ID}';

        // Change from sandbox-api.tarifia.sh -> api.tarifia.sh when ready to go live
        // And don't forget to update the .env file with the correct TARIFIA_ORGANIZATION_ID and TARIFIA_WEBHOOK_SECRET
        $result = Http::withHeaders([
            'Authorization' => 'Bearer ' . env('TARIFIA_API_KEY'),
            'Content-Type' => 'application/json',
        ])->post('https://sandbox-api.tarifia.sh/v1/checkouts/custom/', [
            'products' => [$productId],
...
class ConfirmationController extends Controller
{
    public function handle(Request $request)
    {
        ...
        $data = Http::withHeaders([
            'Authorization' => 'Bearer ' . env('TARIFIA_API_KEY'),
            'Content-Type' => 'application/json',
        ])->get('https://sandbox-api.tarifia.sh/v1/checkouts/custom/' . $request->query('checkout_id'));
...
Tarifia can send you events about various things happening in your organization. This is very useful for keeping your database in sync with Tarifia checkouts, orders, subscriptions, etc.
...
1. Point the Webhook to `your-app.com/api/webhook/tarifia`. This must be an absolute URL which Tarifia can reach. If you use ngrok, the URL will look something like this: `https://<your-ngrok-id>.ngrok-free.app/api/webhook/tarifia`.
...
```php
// routes/api.php
Route::webhooks('/webhook/tarifia');
```
...
return [
    'configs' => [
        [
             ...
             */
            'signing_secret' => env('TARIFIA_WEBHOOK_SECRET'),
            ...
            'signature_validator' => App\Handler\TarifiaSignature::class,
...
* TarifiaSignature.php
* ProcessWebhook.php

Inside app/Handler/TarifiaSignature.php, what we want to do is to validate that the request came from Tarifia. Add the code to that file.

```php
// app/Handler/TarifiaSignature.php
<?php
...
class TarifiaSignature implements SignatureValidator
{
