...
There are two ways to grant initial meter credits in Tarifia:
...
<Steps>
  <Step title="Create a product">
    Open [**Products → New Product**](https://tarifia.sh/to/dashboard/products/new) in the dashboard.
  </Step>
...
- A Tarifia account with an organization
- A meter created with **Sum** aggregation
- A product with the meter attached
- Webhooks enabled
- A Tarifia access token for API calls
...
<Steps>
  ...
  <Step title="Save webhook secret">
    ...
    ```bash Terminal
    TARIFIA_ACCESS_TOKEN="tarifia_pat_..."
    TARIFIA_WEBHOOK_SECRET="whsec_..."
...
```typescript icon="square-js" title="app/api/webhook/tarifia/route.ts"
...
<Steps>
  <Step title="Use the sandbox environment">
    Test your integration in Tarifia's [sandbox environment](/integrate/sandbox) to avoid affecting production data.
  </Step>
