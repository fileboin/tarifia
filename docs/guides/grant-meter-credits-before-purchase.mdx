...
description: "Learn how to grant meter credits to customers before they make any purchase using the Tarifia API."
---
...
1. Create a customer in Tarifia (if they don't exist)
...
<Steps>
  <Step title="Navigate to Meters">
    Open [**Products → Meters**](https://tarifia.sh/to/dashboard/products/meters) in the dashboard.
  </Step>
...
If your customer doesn't already exist in Tarifia, you need to create them first.

### Option A: Create via Dashboard

<Steps>
  <Step title="Navigate to Customers">
    Open [**Customers**](https://tarifia.sh/to/dashboard/customers) in the dashboard.
  </Step>
...
Use the Tarifia SDK or API to create a customer programmatically:
...
await tarifia.customers.create({
...
<Tip>
  Use the `externalId` field to link Tarifia customers with your internal user system. This allows you to reference customers without storing Tarifia's internal ID.
</Tip>

### Using External ID

If you set an `externalId` when creating the customer, you can use it in event ingestion instead of the Tarifia customer ID:
...
const meters = await tarifia.customerMeters.list({
