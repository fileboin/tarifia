...
With standard Tarifia products, one person buys and one person uses — they're the same person. With seat-based products, buying and using are separate concerns, modeled through three distinct entities:
...
- **Benefits are not granted at purchase time.** Seats must be assigned and claimed before benefits are granted. (Exception: when using the default Tarifia confirmation page, the buyer's seat is auto-claimed.)
...
- Tarifia organization with seat-based pricing enabled
- Tarifia SDK installed (`npm install @tarifia-sh/sdk` or `pip install tarifia-sdk`) — make sure it's the latest version
- Basic understanding of Tarifia products and subscriptions
...
<Steps>
  <Step title="Navigate to Products">
    Open the dashboard and [create a new product](https://tarifia.sh/to/dashboard/products/new).
  </Step>
...
Tarifia automatically sends invitation emails to assigned team members with a secure claim link. Once a member claims their seat, benefits are granted immediately.

<Info>
Invitation emails are sent automatically when seats are assigned. The claim flow is fully managed by Tarifia — members click the link, claim their seat, and get immediate access to benefits.
</Info>
...
```typescript
const seat = await tarifia.customerSeats.assign({
...
```typescript
app.post('/webhooks/tarifia', async (req, res) => {
...
```typescript
await tarifia.subscriptions.update({
  id: subscriptionId,
...
- **Communicate clearly** to billing managers about seat assignment: if using the default Tarifia confirmation page, their seat is auto-claimed; if using a custom success URL, they'll need to assign themselves a seat through the portal
...
| Claim link expired | Tokens expire after 24 hours. Resend via the portal or API: `tarifia.customerSeats.resend({ seat_id })` |
