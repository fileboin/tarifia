---
title: "Integrate Tarifia with Next.js"
sidebarTitle: "Next.js"
description: "In this guide, we'll show you how to integrate Tarifia with Next.js."
---
...
[A complete code-example of this guide can be found on GitHub](https://github.com/tarifiasource/tarifia-next).

## Install the Tarifia JavaScript SDK

To get started, you need to install the Tarifia JavaScript SDK and the Tarifia Nextjs helper package. You can do this by running the following command:

```bash Terminal
pnpm install @tarifia-sh/sdk @tarifia-sh/nextjs
```
...
To authenticate with Tarifia, you need to create an access token, and supply it to Next.js using a `TARIFIA_ACCESS_TOKEN` environment variable.

You can create an organization access token from your organization settings.

## Configuring a Tarifia API Client

To interact with the Tarifia API, you need to create a new instance of the `Tarifia` class. This class uses the provided access token to authenticate with the Tarifia API.

```typescript
// src/tarifia.ts
import { Tarifia } from "@tarifia-sh/sdk";

export const api = new Tarifia({
...
import { Checkout } from "@tarifia-sh/nextjs";
...
```bash Terminal
# .env
TARIFIA_ACCESS_TOKEN="tarifia_pat..."
...
```typescript
// src/app/api/webhook/tarifia/route.ts
import { Webhooks } from "@tarifia-sh/nextjs";

export const POST = Webhooks({
	webhookSecret: process.env.TARIFIA_WEBHOOK_SECRET,
