...
Seamlessly integrate Tarifia.sh subscriptions and payments into your Laravel application. This package provides an elegant way to handle subscriptions, manage recurring payments, and interact with Tarifia's API. With built-in support for webhooks, subscription management, and a fluent API, you can focus on building your application while we handle the complexities of subscription billing.

<Note>
This provider is not maintained or officially supported by Tarifia. Use at your own discretion. If you have questions about the provider, please contact the project maintainer.
</Note>
...
```bash Terminal
composer require danestves/laravel-tarifia
```

**Step 2:** Run `:install`:

```bash Terminal
php artisan tarifia:install
```
...
```bash Terminal
php artisan vendor:publish --tag="tarifia-migrations"
```

```bash Terminal
php artisan vendor:publish --tag="tarifia-config"
```

```bash Terminal
php artisan vendor:publish --tag="tarifia-views"
```
...
return [
    /*
    |--------------------------------------------------------------------------
    | Tarifia Access Token
    |--------------------------------------------------------------------------
    |
    | The Tarifia access token is used to authenticate with the Tarifia API.
    | You can find your access token in the Tarifia dashboard > Settings
    | under the "Developers" section.
    |
    */
    'access_token' => env('TARIFIA_ACCESS_TOKEN'),
    ...
    |--------------------------------------------------------------------------
    |
    | The Tarifia webhook secret is used to verify that the webhook requests
    | are coming from Tarifia. You can find your webhook secret in the Tarifia
    ...
    |
    */
    'webhook_secret' => env('TARIFIA_WEBHOOK_SECRET'),
    ...
    |--------------------------------------------------------------------------
    |
    | This is the base URI where routes from Tarifia will be served
    | from. The URL built into Tarifia is used by default; however,
    | you can modify this path as you see fit for your application.
    |
    */
    'path' => env('TARIFIA_PATH', 'tarifia'),
    ...
    | from a checkout session in your Tarifia account.
    |
    */
    'redirect_url' => null,
    ...
    |
    */
    'currency_locale' => env('TARIFIA_CURRENCY_LOCALE', 'en'),
];
```
...
Configure your access token. Create a new token in the Tarifia Dashboard > Settings > Developers and paste it in the `.env` file.

- https://sandbox.tarifia.sh/dashboard/ORG_SLUG/settings (Sandbox)
- https://tarifia.sh/dashboard/ORG_SLUG/settings (Production)

```bash Terminal
TARIFIA_ACCESS_TOKEN="<your_access_token>"
```

### Webhook Secret

Configure your webhook secret. Create a new webhook in the Tarifia Dashboard > Settings > Webhooks.

- https://sandbox.tarifia.sh/dashboard/ORG_SLUG/settings/webhooks (Sandbox)
- https://tarifia.sh/dashboard/ORG_SLUG/settings/webhooks (Production)
...
```bash Terminal
TARIFIA_WEBHOOK_SECRET="<your_webhook_secret>"
```
...
```php
use Danestves\LaravelTarifia\Billable;
...
Tarifia includes a JavaScript script that you can use to initialize the [Tarifia Embedded Checkout](https://tarifia.sh/docs/features/checkout/embed). If you going to use this functionality, you can use the `@tarifiaEmbedScript` directive to include the script in your views inside the `<head>` tag.
...
This package includes a webhook handler that will handle the webhooks from Tarifia.
...
Or if you're using Laravel v11 and up, you should exclude `tarifia/*` in your application's `bootstrap/app.php` file:
...
This package includes a list of commands that you can use to retrieve information about your Tarifia account.

| Command                      | Description                                |
| ---------------------------- | ------------------------------------------ |
| `php artisan tarifia:products` | List all available products with their ids |
...
```blade
<x-tarifia-button :checkout="$checkout" />
```
...
```blade
<x-tarifia-button :checkout="$checkout" data-tarifia-checkout-theme="dark" />
```
...
```php
public function tarifiaName(): ?string; // default: $model->name
public function tarifiaEmail(): ?string; // default: $model->email
```
...
Customers can update their personal information (e.g., name, email address) by accessing their [self-service customer portal](https://tarifia.sh/docs/features/customer-portal). To redirect customers to this portal, call the `redirectToCustomerPortal()` method on your billable model (e.g., the User model).
...
```blade
@if ($order->refunded())
    Order {{ $order->tarifia_id }} was refunded on {{ $order->refunded_at->toFormattedDateString() }}
@endif
```
...
If a recurring payment fails, the subscription will become past due. This indicates that the subscription is still valid, but your customer's payments will be retried automatically over a 21-day period.
...
Tarifia does not offer immediate cancellation. To resume a subscription while it is still in its grace period, use the resume method.
...
Tarifia can send webhooks to your app, allowing you to react. By default, this package handles the majority of the work for you. If you have properly configured webhooks, it will listen for incoming events and update your database accordingly. We recommend activating all event kinds so you may easily upgrade in the future.

#### Webhook Events

- `Danestves\LaravelTarifia\Events\BenefitGrantCreated`
- `Danestves\LaravelTarifia\Events\BenefitGrantUpdated`
- `Danestves\LaravelTarifia\Events\BenefitGrantRevoked`
- `Danestves\LaravelTarifia\Events\OrderCreated`
- `Danestves\LaravelTarifia\Events\OrderRefunded`
- `Danestves\LaravelTarifia\Events\SubscriptionActive`
- `Danestves\LaravelTarifia\Events\SubscriptionCanceled`
- `Danestves\LaravelTarifia\Events\SubscriptionCreated`
- `Danestves\LaravelTarifia\Events\SubscriptionRevoked`
- `Danestves\LaravelTarifia\Events\SubscriptionUpdated`
...
use Danestves\LaravelTarifia\Events\WebhookHandled;

class TarifiaEventListener
{
    /**
     * Handle received Tarifia webhooks.
     */
...
The [Tarifia documentation](https://tarifia.sh/docs/integrate/webhooks/events) includes an example payload.
...
use App\Listeners\TarifiaEventListener;
...
Please see [CHANGELOG](https://github.com/danestves/laravel-tarifia/blob/main/CHANGELOG.md) for more information on what has changed recently.

## License

The MIT License (MIT). Please see [License File](https://github.com/danestves/laravel-tarifia/blob/main/LICENSE.md) for more information.
