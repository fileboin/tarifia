...
## @tarifia-sh/better-auth

A [Better Auth](https://github.com/better-auth/better-auth) plugin for integrating [Tarifia](https://tarifia.sh) payments and subscriptions into your authentication flow.
...
- [Handle Tarifia Webhooks securely with signature verification](#webhooks-plugin)
- [Customer Portal](#portal-plugin)

## Examples

- [With Next.js, Better Auth and Cloudflare Workers](https://github.com/tarifiasource/examples/tree/main/with-nextjs-better-auth-cloudflare-workers)

## Installation

Install the required Better Auth and Tarifia packages using the following command:

<Tabs>
  <Tab title="npm">
    ```bash Terminal
    npm install better-auth @tarifia-sh/better-auth @tarifia-sh/sdk
    ```
  </Tab>
  <Tab title="yarn">
    ```bash Terminal
    yarn add better-auth @tarifia-sh/better-auth @tarifia-sh/sdk
    ```
  </Tab>
  <Tab title="pnpm">
    ```bash Terminal
    pnpm add better-auth @tarifia-sh/better-auth @tarifia-sh/sdk
    ```
  </Tab>
  <Tab title="bun">
    ```bash Terminal
    bun add better-auth @tarifia-sh/better-auth @tarifia-sh/sdk
    ```
  </Tab>
</Tabs>

## Integrate Tarifia with BetterAuth

<Steps>
  <Step title="Configure Tarifia Access Token">
    Go to your Tarifia Organization Settings, create an Organization Access Token, and add it to the environment variables of your application.

    ```bash .env
    TARIFIA_ACCESS_TOKEN=...
    ```

  </Step>
  <Step title="Configure BetterAuth Server">
    The Tarifia plugin comes with it's own set of plugins to add functionality to your stack:
    ...
    - **webhooks** - Listen for relevant Tarifia webhooks

    ```typescript icon="square-js" auth.ts
    import { betterAuth } from "better-auth";
    import { tarifia, checkout, portal, usage, webhooks } from "@tarifia-sh/better-auth"; // [!code ++]
    import { Tarifia } from "@tarifia-sh/sdk"; // [!code ++]

    const tarifiaClient = new Tarifia({ // [!code ++]
        accessToken: process.env.TARIFIA_ACCESS_TOKEN, // [!code ++]
        // Use 'sandbox' if you're using the Tarifia Sandbox environment
    ...
    const auth = betterAuth({
        ...
        plugins: [
            tarifia({ // [!code ++]
                client: tarifiaClient, // [!code ++]
                createCustomerOnSignUp: true, // [!code ++]
                use: [ // [!code ++]
                    checkout({ // [!code ++]
                        products: [ // [!code ++]
                            { // [!code ++]
                                productId: "123-456-789", // ID of Product from Tarifia Dashboard // [!code ++]
                    ...
                    usage(), // [!code ++]
                    webhooks({ // [!code ++]
                        secret: process.env.TARIFIA_WEBHOOK_SECRET, // [!code ++]
    ...
    #### Tarifia Plugin Configuration Options
    ...
    const auth = betterAuth({
      ...
      plugins: [
        tarifia({
          ...
          use: [ // [!code ++]
            // This is where you add Tarifia plugins // [!code ++]
          ], // [!code ++]
        }),
      ],
    });
    ...
    - `client` (required): Tarifia SDK client instance
    - `createCustomerOnSignUp` (optional): Automatically create a Tarifia customer when a user signs up
    - `getCustomerCreateParams` (optional): Custom function to provide additional customer creation metadata
    - `use` (optional): Array of Tarifia plugins to enable specific functionality (checkout, portal, usage, and webhooks)

  </Step>
  <Step title="Configure BetterAuth Client">
    You will be using the BetterAuth Client to interact with the Tarifia functionalities.

    ```typescript icon="square-js" auth-client.ts
    import { createAuthClient } from "better-auth/react";
    import { tarifiaClient } from "@tarifia-sh/better-auth/client"; // [!code ++]
    import { organizationClient } from "better-auth/client/plugins"; // [!code ++]

    // All Tarifia plugins, etc. should be attached to BetterAuth server
    export const authClient = createAuthClient({ // [!code ++]
      plugins: [tarifiaClient()], // [!code ++]
    }); // [!code ++]
    ```
...
Enable the `createCustomerOnSignUp` [Tarifia plugin configuration option](#tarifia-plugin-configuration-options) to automatically create a new Tarifia Customer when a new User is added in the BetterAuth database.

All new customers are created with an associated `externalId`, i.e. the ID of your User in the Database. This skips any Tarifia to User mapping in your database.

## Sync Customer deletion

To add user deletion logic with external Tarifia customer deletion in BetterAuth, extend the user config of your betterAuth setup with the deleteUser option and an afterDelete hook. Here’s how to integrate this alongside your Tarifia plugin and automatic customer creation:
  ...
  user: {
    ...
    deleteUser: {
      ...
      afterDelete: async (user, request) => {
        // [!code ++]
        await tarifia.customers.deleteExternal({
          // [!code ++]
...
[Source code](https://github.com/tarifiasource/tarifia-adapters/blob/main/packages/tarifia-betterauth/src/plugins/checkout.ts)
...
<Steps>
  <Step title="Use Checkout Plugin">
    Update the `use` property of the Tarifia plugin for BetterAuth client to have the `checkout` plugin.

    ```typescript icon="square-js" Checkout Plugin Example
    import {
      tarifia,
      checkout // [!code ++]
    } from "@tarifia-sh/better-auth";
  ...
  <Step title="Create checkouts using BetterAuth client">
    ...
    - **`products`** (optional): An array of Tarifia Product IDs.
    ...
    ```typescript icon="square-js" BetterAuth Checkout with Tarifia Example
    ...
    ```typescript icon="square-js" BetterAuth Checkout with Tarifia Organization Example
    const organizationId = (await authClient.organization.list())?.data?.[0]?.id,

    await authClient.checkout({
        // Any Tarifia Product ID can be passed here
...
[Source code](https://github.com/tarifiasource/tarifia-adapters/blob/main/packages/tarifia-betterauth/src/plugins/usage.ts)
...
Tarifia's Usage Based Billing builds entirely on event ingestion. Ingest events from your application, create Meters to represent that usage, and add metered prices to Products to charge for it.
...
Because `createCustomerOnSignUp: true` sets the Tarifia customer's `externalId` to the BetterAuth user ID, you can pass `externalCustomerId: session.user.id` when ingesting — no extra lookup is needed.

The example below uses the Tarifia SDK directly from a server route handler. The same `tarifiaClient` instance you configured in `auth.ts` is reused:

```typescript icon="square-js" app/api/ai/video/route.ts (Next.js App Router)
import { auth } from "@/lib/auth";
import { tarifiaClient } from "@/lib/tarifia";
...
export async function POST(request: Request) {
  ...
  await tarifiaClient.events.ingest({
    events: [
      {
        name: "ai-video",
...
The `usage` plugin also registers a BetterAuth endpoint that is callable from the client as `authClient.usage.ingestion(...)`. It forwards to Tarifia from the BetterAuth server and attaches the authenticated user automatically:
...
[Source code](https://github.com/tarifiasource/tarifia-adapters/blob/main/packages/tarifia-betterauth/src/plugins/webhooks.ts)

The `webhooks` plugin can be used to capture incoming events from your Tarifia organization.

To set up the Tarifia `webhooks` plugin with the BetterAuth client, follow the steps below:

<Steps>
  <Step title="Configure Webhook Endpoints in Tarifia">
    Configure a Webhook endpoint in your Tarifia Organization Settings page by following [this guide](/integrate/webhooks/endpoints). Webhook endpoint is configured at `/api/auth/tarifia/webhooks`.
  </Step>
  <Step title="Add the Webhook Secret">
    Add the obtained webhook secret to your application environment as an environment variable (to be used as `process.env.TARIFIA_WEBHOOK_SECRET`):
  
    ```bash .env
    TARIFIA_WEBHOOK_SECRET="..."
    ```
  </Step>
...
The `webhooks` plugin allows you to invoke handlers for all Tarifia webhook events:
...
[Source code](https://github.com/tarifiasource/tarifia-adapters/blob/main/packages/tarifia-betterauth/src/plugins/portal.ts)
...
The following method will redirect the user to the Tarifia Customer Portal, where they can see their orders, purchases, subscriptions, benefits, etc.
...
Learn more about the Tarifia Customer State [in the Tarifia Docs](https://tarifia.sh/docs/integrate/customer-state).
