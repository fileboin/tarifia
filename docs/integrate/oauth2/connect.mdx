...
```
https://tarifia.sh/oauth2/authorize?
  response_type=code
  &client_id=CLIENT_ID
...
<ParamField path="response_type=code" type="string" required>
  ...
  the only one supported by Tarifia.
</ParamField>
...
Tarifia issues **user-scoped** access tokens, tied to the user granting access. On the consent screen, the user can optionally **limit the token to specific organizations**: by default it can access all of their organizations (including ones they join later), but if they select specific ones, it's restricted to those. Access always intersects with the user's current membership, so it adjusts automatically if they leave an organization. You can check which organizations a token is limited to by [introspecting it](/api-reference/oauth2/connect/introspect-token) — the response includes an `organizations` array (an empty array means all of them).
...
{
  "token_type": "Bearer",
  "access_token": "tarifia_at_XXX",
  "expires_in": 864000,
  "refresh_token": "tarifia_rt_XXX",
  "scope": "openid email",
  "id_token": "ID_TOKEN"
}
```
...
```bash Terminal
curl -X GET https://api.tarifia.sh/v1/oauth2/userinfo \
  -H 'Authorization: Bearer tarifia_at_XXX'
```
