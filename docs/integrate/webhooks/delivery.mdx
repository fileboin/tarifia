...
You now need to setup a route handler for the endpoint registered on Tarifia to
...
import { validateEvent, WebhookVerificationError } from '@tarifia-sh/sdk/webhooks'
  ...
  (req: Request, res: Response) => {
    try {
      const event = validateEvent(
        req.body,
        req.headers,
        process.env['TARIFIA_WEBHOOK_SECRET'] ?? '',
      )
...
from tarifia_sdk.webhooks import validate_event, WebhookVerificationError
...
def webhook():
    try:
        event = validate_event(
            ...
            secret=os.getenv('TARIFIA_WEBHOOK_SECRET', ''),
        )
...
Both examples above expect an environment variable named `TARIFIA_WEBHOOK_SECRET`
...
To re-enable a disabled endpoint, open your organization's [webhook settings](https://tarifia.sh/to/dashboard/settings/webhooks) and manually enable it. Before re-enabling, ensure your endpoint is properly configured and reachable to avoid repeated disabling.

## Troubleshooting

### Not receiving webhooks

Seeing deliveries on Tarifia, but not receiving them on your end? Below are some
...
- Try adding trailing `/` to the URL on Tarifia. Often `/foo` is resolved to
  `/foo/` by frameworks.

`HTTP 3xx`

Redirect responses (301, 302, 307, etc.) are treated as failures. Tarifia does not follow redirects for webhook deliveries. Update your webhook URL to the final destination URL to avoid redirects.
