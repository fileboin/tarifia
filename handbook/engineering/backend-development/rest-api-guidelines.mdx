...
description: "Best practices and conventions for designing and implementing RESTful APIs at Tarifia"
---

This document outlines the guidelines and principles we follow when designing and implementing RESTful APIs at Tarifia. Adhering to these standards ensures consistency, maintainability, and a better developer experience for both internal and external users of our APIs.
...
- Always use `tarifia.kit.Schema` (or `TimestampedSchema` when timestamps exist)
...
- Validation errors should return `422 Unprocessable Entity` in FastAPI format via `TarifiaRequestValidationError`
...
**Inherit from `tarifia.kit.Schema`**

```py
# ✅ DO
from tarifia.kit import Schema
...
**If the underlying data model includes `created_at` or `updated_at` fields, inherit from `tarifia.kit.TimestampedSchema`**

```py
# ✅ DO
from tarifia.kit import TimestampedSchema
...
- On validation errors, return `422 Unprocessable Entity` with FastAPI-style details (`TarifiaRequestValidationError`).
