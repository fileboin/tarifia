...
description: "How Tarifia's financial ledger entities work: Account (transaction ledger) and PayoutAccount (payout processor configuration)."
---

## Overview

Tarifia's financial architecture is split across two distinct entities:
...
erDiagram
    ...
    Transaction }o--o| Account : "account_id (nullable = Tarifia's own)"
...
- **Multiple organisations** can share a single `PayoutAccount` (e.g. a company with several Tarifia orgs pointing to one Stripe Connect account).
- **Accounts are created immediately** on org creation — no held balances, no pending state.

## `Account` Model

**`server/tarifia/models/account.py` · table `accounts`**
...
```python
currency: str              # Always "usd" (Tarifia's settlement currency)
...
`credit_balance` is **not** the merchant's earned revenue balance. It tracks **promotional fee credits** explicitly granted by Tarifia (manually by an admin, or via a campaign). Each credit is stored as an `AccountCredit` row; `credit_balance` is a denormalized running sum for fast reads.
...
**`server/tarifia/models/payout_account.py` · table `payout_accounts`**
...
**`server/tarifia/models/organization.py`**
...
**`server/tarifia/models/transaction.py` · table `transactions`**

```python
account_id: UUID | None    # NULL → Tarifia's own transaction; NOT NULL → merchant/seller
...
- `account_id IS NULL` → transaction is Tarifia's own (e.g. collected fees, outgoing transfers)

**Transaction types:** `payment`, `processor_fee`, `refund`, `refund_reversal`, `dispute`, `dispute_reversal`, `balance`, `payout`, `payout_reversal`

The future goal is to extend `Account` to represent customers and Tarifia subsidiaries too, enabling the full money trail to be traced in a single table:

```mermaid
flowchart LR
    C[Customer Account] -->|payment| P[Tarifia Account\nMerchant of Record]
...
**`server/tarifia/models/payout.py` · table `payouts`**
...
currency: str              # "usd" (Tarifia's side)
amount: int                # cents
