...
description: "Understanding and handling operational errors in Tarifia backend development"
---
...
1. **Create a matcher function** in `server/tarifia/operational_errors.py`:
...
- **API requests**: `OperationalErrorMiddleware` in `server/tarifia/middlewares.py`
- **Background jobs**: `OperationalErrorMiddleware` in `server/tarifia/worker/_broker.py`
