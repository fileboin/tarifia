---
title: "Metadata"
description: "A shared mechanism letting users attach arbitrary key-value data to Tarifia models."
---

Metadata is a shared mechanism that lets Tarifia users attach arbitrary key-value pairs to supported resources (customers, orders, subscriptions, products, etc.). It is entirely for the benefit of integrators — Tarifia itself does not use these values — and is exposed as a plain `metadata` field in every API request and response.
...
- Users are likely to need to cross-reference Tarifia records with their own systems (e.g. storing an internal record ID, a feature flag, or a campaign tag).

Do **not** use `user_metadata` to store Tarifia-internal data; use dedicated columns for that.
...
**`add_metadata_query_schema`** is called once, at application startup, inside the custom `openapi()` function in `tarifia/openapi.py`. It injects a reusable `MetadataQuery` component into the global `components/schemas` section of the OpenAPI document:
