...
description: "How to use task debouncing to avoid duplicate task execution in Tarifia's background worker system."
---

Task debouncing is a mechanism used in Tarifia's background worker system to prevent duplicate execution of tasks that are triggered multiple times in a short amount of time. This is particularly useful for operations that might be triggered by multiple events or user actions.
...
```python
from tarifia.worker import actor
...
- `tarifia_task_debounced_total`: Counter of tasks that were skipped due to debouncing
- `tarifia_task_debounce_delay_seconds`: Histogram of delays between first enqueue and execution
