---
title: "Authorization"
description: "How authorization works in Tarifia: authentication vs authorization, PolicyGuard, policies, and org-scoped data access."
---

## Overview

Authorization in Tarifia is split into two layers:
...
- **Authorization** (`server/tarifia/authz/`): "Are you allowed to do this?" — checks org membership, evaluates policies, enforces access control.
...
```python
from tarifia.authz.dependencies import AuthorizeFinanceRead
...
Each guard is paired with a policy and exposed as a named dependency in `server/tarifia/authz/dependencies.py`. Endpoints depend on these names rather than wiring guards and policies themselves. For example:
...
See `server/tarifia/authz/dependencies.py` for the full list.

## Policies

Policies are async functions in `server/tarifia/authz/policies/`. Each policy receives the database session, auth subject, and organization, and returns either `True` (allowed) or a denial reason string.

```python
# server/tarifia/authz/policies/finance.py
async def can_read(
...
For these, use one of the user-personal `Authorize*` aliases from `server/tarifia/authz/dependencies.py`:
...
- `WebUserSession` (in `server/tarifia/auth/dependencies.py`) — pure authentication for the `Web*` aliases. Accepts only web-session users; rejects API tokens.
...
1. **Choose or create a policy function** in `server/tarifia/authz/policies/`
2. **Create a typed dependency** in `server/tarifia/authz/dependencies.py` using the appropriate guard variant
