---
title: "Platform API"
description: "Proposal for a Platform API to enable partners to create and manage Tarifia Organizations on behalf of their users."
---
...
The Platform API enables partners to create and manage Tarifia Organizations on behalf of their users, allowing seamless integration of monetization features into third-party platforms. This document outlines the technical architecture, authentication flow, and claim process for the Platform API.
...
3. **Token Exchange**: JWT is sent to Tarifia's OAuth2 token endpoint
4. **Validation**: Tarifia validates the JWT using the platform's public key
5. **Access Token**: Tarifia returns an Organization Access Token
...
```json
{
  ...
  "aud": "tarifia:platform-organization", // Audience: organization operations
...
```json
{
  ...
  "aud": "tarifia:platform", // Audience: platform operations
  "iat": 1735689600, // Issued at timestamp
  "nbf": 1735689600, // Not before timestamp
  "exp": 1735776000 // Expiration timestamp
}
...
2. **Audience Check**: Ensure `aud` = `tarifia:platform-organization`
...
1. **JWT Validation**: Verify signature using platform's JWKS
2. **Audience Check**: Ensure `aud` = `tarifia:platform`
...
1. **Claim Token Generation**:
   ...
   - Tarifia generates a short-lived, single-use claim token
   - Token contains: `organization_id`, `platform_id`, expiration

2. **User Onboarding**:
   - Platform presents claim token to their user
   - User visits Tarifia's onboarding flow with claim token
   - User creates Tarifia account (or logs in)

3. **Organization Claiming**:
   - User submits claim token during onboarding
   - Tarifia validates token and links User to Organization
...
- `aud` is `tarifia:platform` (vs `tarifia:platform-organization` for org-specific operations)
...
```bash
curl -X POST https://api.tarifia.sh/v1/oauth2/token \
...
- `POST /v1/oauth2/token` - Exchange JWT for access tokens
  - `grant_type: platform` with `aud: tarifia:platform` → Platform Access Token
  - `grant_type: platform` with `aud: tarifia:platform-organization` → Organization Access Token
...
- Standard Tarifia API endpoints (products, customers, subscriptions, etc.)

# Appendix: Sandbox as a Platform

Maybe a crazy idea, but we could consider the Tarifia Sandbox as a Platform on the Production environment. This would enable us to implement processes to automatically create an Organization in Production from a Sandbox!
