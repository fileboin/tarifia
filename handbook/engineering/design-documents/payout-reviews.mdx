...
- No signal-based gating on `ACTIVE` orgs (that fits into [#121](https://github.com/tarifiasource/feedback/issues/121)).
...
- **Naming.** We use `held` for the status. Note Tarifia already uses "Held balance" and an `on_hold` transaction availability status for balance concepts, so keep the payout `held` status mentally separate from those.
...
Tarifia does not have a balance column anywhere. The available balance is computed on demand by `TransactionService.get_summary`, as a filtered `SUM(Transaction.amount)` over the `transactions` table for the merchant's account. A transaction row counts toward the available balance if any of these is true:
...
1. **Fee debits.** `create_payout_fees_balances` ([server/tarifia/transaction/service/platform_fee.py](server/tarifia/transaction/service/platform_fee.py)) inserts up to three balance pairs, one per applicable fee (`account`, `cross_border_transfer`, `payout`). Each pair is a merchant-side debit and a Tarifia-side credit. The merchant-side rows count toward the available balance via the `platform_fee_type` arm.
...
The `payout_status_update_trigger` Postgres trigger ([server/tarifia/models/payout_attempt.py](server/tarifia/models/payout_attempt.py)) keeps `payouts.status` in sync with the latest `payout_attempt`. It already has one early-return guard preserving `canceled`:
...
Both fixes live in [server/tarifia/payout/repository.py](server/tarifia/payout/repository.py) and land in PR 2:
