...
This RFC proposes a date-based API versioning strategy for Tarifia with the following characteristics:
...
- **Versioning mechanism**: `Tarifia-Version` header, defaulting to Current version
...
Currently, Tarifia lacks a formal API versioning strategy. Our current approach is "best-effort", where we _try_ to limit breaking changes and maintain backward compatibility, but we have no guarantees or clear policies. This leads to several issues:
...
Tarifia dashboard and mobile app will always use the **Next** version, so it can benefit from latest features immediately. Breaking changes are less disruptive for internal clients since we control both sides of the contract.
...
```http
Tarifia-Version: 2026-01
```

#### Default Behavior

When the `Tarifia-Version` header is omitted, requests are routed to the **Current** version.

<Warning>
  The default version changes over time as new versions are released. **Clients
  should explicitly set the `Tarifia-Version` header to avoid unexpected
...
```py
from tarifia.v2026_07 import Tarifia
```
...
Custom routing classes will then take care to create sub-FastAPI apps that expose the right endpoints and schemas for each version. The main app will route requests to the appropriate sub-app based on the `Tarifia-Version` header.
...
class VersionRoute(BaseRoute):
    ...
    def matches(self, scope: Scope) -> tuple[Match, Scope]:
        if scope["type"] in ("http", "websocket"):
            ...
            raw_version = headers.get("tarifia-version")
            try:
                request_version = (
