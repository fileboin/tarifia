...
- Tarifia staff can also fetch a customer's receipt from backoffice. Merchants can fetch it through the dashboard API.
...
Each Order gets at most one receipt. Tarifia Orders have at most one succeeded `Payment`, so per-Order and per-Payment collapse into the same shape. The receipt is allocated when the Order transitions to paid. The PDF itself is rendered lazily on first download and re-rendered whenever the payment state changes (typically a refund).
...
| Backoffice (Tarifia staff) | Existing backoffice auth, read-only | mirror of the customer-portal endpoint |
...
```
python -m tarifia.scripts.backfill_receipts <organization_id>
```
...
| 2 | New `tarifia/receipt/` module, Terraform for the new bucket, allocation hook, render actor with Redis lock, refund hook, metrics. `receipts_enabled` defaults to `false` everywhere |
