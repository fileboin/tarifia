...
In this new era of AI-based apps, usage-based billing is becoming the new standard business model. Previously, we added support for usage-based billing in Tarifia, using a event-based system to track usage and bill customers accordingly. The usage is accrued over the subscription cycle and billed at the end of the cycle.
...
In this document, we'll explore how we can implement a cash balance system in Tarifia, and how it can complement our existing usage-based billing system. We call that **Wallets**.
...
A customer signs up on a merchant's app. Using the API, the merchant creates a Customer on Tarifia and creates a subscription for their usage-based product. A new Wallet is created for the Customer, with a 0 balance.

To top-up the Wallet, the merchant redirects the Customer to the Tarifia's Customer Portal. From there, the Customer can add a payment method and top-up their Wallet with a desired amount of money. The Customer can top-up as many times as they want, and the balance is cumulative. Each top-up creates an Order in the merchant's Organization, and the funds are transferred to the Organization's Account, minus Tarifia fees.

When the Customer uses the merchant's app, the app tracks the usage and reports it to Tarifia using the Event Ingestion API. Tarifia deducts the cost of the usage from the Customer's Wallet balance.
