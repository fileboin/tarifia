...
Introduce role-based access control for users on a Tarifia organization. Three roles ship in this iteration — `owner`, `admin`, and `member` — replacing today's implicit binary where org-level admin capability (member management, organization deletion, payout-account selection, Finance and payouts) is gated by `Account.admin_id`: the single user who is recorded as the admin on the org's Tarifia-side billing `Account`, today set at org creation.
...
Tarifia's authorization model has two distinct layers:
...
- **Notification routing and the Tarifia-for-Tarifia owner-side relationship** — re-anchored on the user holding `owner`. The user identified by these flows is unchanged; only the lookup path changes.
