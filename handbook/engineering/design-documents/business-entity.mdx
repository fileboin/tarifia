...
2. Tarifia Checkout: Buyer enters email + card
3. Tarifia auto-creates individual customer
4. Post-purchase: Customer manages subscription via Customer Portal

#### Workflow B: Public Checkout (B2B - Current seats flow)
1. Customer visits merchant site, clicks checkout link
2. Tarifia Checkout: Buyer enters email + card
3. Tarifia auto-creates billing manager
...
2. Merchant creates Tarifia customer + business inside Tarifia via API
3. Billing manager clicks checkout in merchant app
4. Tarifia Checkout: Pre-filled information, enters card
...
5. **Tarifia Developer Experience** (weight: 3) - Avoid excessive filtering or complex queries
...
| Tarifia Dev Experience    | 🟢 3  | Clean queries, no type filtering, clear separation |
...
| Tarifia Dev Experience    | 🟡 2  | Requires type filtering, mitigated by SQLAlchemy polymorphism |
...
| Tarifia dev experience    | 3      | 🟢 3 (9)               | 🟡 2 (6)               |
...
We're introducing a new **Member** entity to improve how Tarifia handles team subscriptions and usage-based billing. This change provides:
...
**Problem we're solving**: When a user (e.g., Alice) works for multiple companies (Acme Corp and Slack Inc), and both companies purchase your product, Tarifia doesn't know which company to bill when Alice generates usage events. This causes billing ambiguity. This will allow other futures in the comming weeks, like better analytics and insights.
...
**Before** (current):
```typescript
const session = await tarifia.benefits.grants({
  externalCustomerId: "cust_ben_123"
});
```
...
- [ ] **Update SDK to latest version**
  - TypeScript: `npm install @tarifia-sh/sdk@latest`
  - Python: `pip install --upgrade tarifia-sdk`
...
| **Week 1-2** | One-on-one migration support | Work with Tarifia team if issues arise |
...
- Email support@tarifia.sh with questions

**During migration**:
- Schedule 1-on-1 call if needed.

**After migration**:
- Report issues immediately: support@tarifia.sh
- Check status page: status.tarifia.sh
...
This appendix shows how merchants integrate with Tarifia for common workflows, comparing **Option 1 (Member Model)** vs **Option 2 (Customer Type + Member)** side-by-side.
...
```http
POST https://api.tarifia.sh/v1/customers
Authorization: Bearer tarifia_secret_...
...
POST https://api.tarifia.sh/v1/customers/cust_acme_123/members
Authorization: Bearer tarifia_secret_...
...
POST https://api.tarifia.sh/v1/checkouts
Authorization: Bearer tarifia_secret_...
...
{
  "id": "checkout_abc",
  "url": "https://tarifia.sh/checkout/abc",
  "customer_id": null  // Created after payment
}
```

```http
# Step 2: Customer completes checkout (Tarifia hosted)
# → Tarifia auto-creates Customer + Member
...
# Tarifia automatically creates BenefitGrant linked to ben_alice_789
```
...
```http
# Step 2: Customer completes checkout (Tarifia hosted)
# → Tarifia auto-creates Customer with type="individual"
...
# Tarifia automatically creates BenefitGrant linked to cust_alice_456
```
...
{
  "id": "checkout_def",
  "url": "https://tarifia.sh/checkout/def",
  "customer_id": "cust_acme_123"
}
```
...
# → Tarifia auto-creates member for billing@acme.com
```
...
POST https://api.tarifia.sh/v1/subscriptions/sub_acme_456/seats
Authorization: Bearer tarifia_customer_session_... // ben_billing_789 session
...
```http
# Step 5: Beneficiaries claim seats
POST https://api.tarifia.sh/v1/subscriptions/sub_acme_789/seats
Authorization: Bearer tarifia_customer_session_... // cust_acme_456 session
...
https://buy.tarifia.sh/tarifia_cl_gon2ISU4r7oYyjVaxXlRVUv5MKh1H3c1gCxKn1zZGHA
...
```http
# Merchant shares public checkout link
https://tarifia.sh/org_abc/products/prod_team_plan

# Customer clicks, enters email, completes purchase
# → Tarifia detects seat-based product, auto-creates:
...
GET https://api.tarifia.sh/v1/customer-portal/downloadables
Authorization: Bearer tarifia_customer_session_...
...
GET https://api.tarifia.sh/v1/benefit-grants?member_id=ben_alice_acme_123
Authorization: Bearer tarifia_member_session_...
```
...
```http
# Download benefit
GET https://api.tarifia.sh/v1/benefits/benefit_theme_download/download
Authorization: Bearer tarifia_customer_session_...
```
...
POST https://api.tarifia.sh/v1/customer-sessions
Authorization: Bearer tarifia_secret_...
...
```json
{
  "token": "tarifia_cst_abc123xyz",
  "customer_id": "cust_bob_123",
  "member_id": "ben_bob_default_456",  // Auto-resolved from customer
  "return_url": "https://tarifia.sh/org_merchant/portal?token=tarifia_cst_abc123xyz",
...
```json
{
  "token": "tarifia_cst_def456",
  "member_id": "ben_jane_789",
  "customer_id": "cust_acme_123",
  "url": "https://tarifia.sh/portal?token=tarifia_cst_def456",
  "expires_at": "2025-12-01T10:00:00Z"
}
```
...
```http
# Option A: Using customer_id (B2C backward compatible)
POST https://api.tarifia.sh/v1/events
Authorization: Bearer tarifia_secret_...
...
**What happens:**
- Tarifia checks if cust_alice_123 is a member of any businesses
...
The user access: https://tarifia.sh/petru-test/portal/request
