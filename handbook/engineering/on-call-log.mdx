...
Running out of memory continuously on the workers, causing a crash loop after getting an influx of exceptions in the processing of them. [Read more discussion](https://tarifia-olw4768.slack.com/archives/C09T5UDPF2Q). Potentially related to the Sentry integration in Dramatiq being reenabled. We have now disabled it again which can be monitored if this continues to happen or if it stops now.


#### 2025-11-15

Webhooks were slow to process. Most likely due to an overload of other events causing a delay in processing. [Read more details](https://tarifia-olw4768.slack.com/archives/C093CJXJXPT/p1763221839002159).
...
- 20:18: [Number of processes/threads of the worker increased](https://github.com/tarifiasource/tarifia/commit/61e5f973ab46c55b1c11a9da744695b760884ad5) with no effect
- 20:32: [Patch deployed for task `customer_meter.update_customer` to skips retries and exits early if lock is occupied](https://github.com/tarifiasource/tarifia/commit/a67530c3d169920fdbad7fc9437bea94d88b9b96)
...
As an immediate fix for the issue: [we're updating our logic so that frequent `customer_meter.update_customer` calls are deduped](https://github.com/tarifiasource/tarifia/issues/7146).

We are also looking into another way to set up a [high-priority queue](https://github.com/tarifiasource/tarifia/issues/7149) now that we've learned that Dramatiq's built-in priority does nothing on Redis.
...
- Fixed the tests & CI on `tarifia-adapters`
