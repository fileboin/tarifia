---
title: "Tarifia Cycle"
description: "How to configure Tarifia Cycle, the Slack bot that announces PRs and rotates the primary reviewer"
---

Tarifia Cycle is the small Slack bot that posts PR threads in `#pull-requests` and picks a primary reviewer whenever a team is requested for review. It runs on Vercel at [tarifia-cycle.vercel.app](https://tarifia-cycle.vercel.app), with a tiny admin UI for the bits we need to tweak day to day.

This guide covers what the admin panel does and the most common tasks you will run from it. The implementation lives in [`tarifiasource/cycle`](https://github.com/tarifiasource/cycle).
...
Go to [tarifia-cycle.vercel.app/admin](https://tarifia-cycle.vercel.app/admin).

### Sign in with GitHub

Click **Sign in with GitHub**. You must be a member of `@tarifiasource` — Cycle calls the GitHub API with your token to verify org membership before issuing a session. Anyone in the org can sign in and edit configuration; there are no further role checks.
...
A newline-separated list of `owner/repo` slugs that Cycle should listen to (e.g. `tarifiasource/tarifia`). Cycle ignores webhooks for any repo not in this list, so adding a new repo here is the switch that turns Cycle on for it.

<Note>
  Webhooks are configured at the GitHub App level (separately from this panel). If you add a
  brand-new repo and Cycle is not posting anything, double-check that the **Tarifia Cycle** GitHub App
...
1. Make sure the **Tarifia Cycle** GitHub App is installed on the repo (org admins can do this).
...
- Source: [`tarifiasource/cycle`](https://github.com/tarifiasource/cycle)
- Admin URL: https://tarifia-cycle.vercel.app/admin
- Storage: Upstash Redis (configured via Vercel env vars)
- Auth: GitHub OAuth, restricted to `@tarifiasource` org members, signed-cookie sessions (7-day TTL)
