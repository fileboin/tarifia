...
1. Clone the `tarifia-js` repository
2. In `tarifia-js/.speakeasy/workflow.yaml` change `https://api.tarifia.sh/openapi.json` to `http://127.0.1:8000/openapi.json`
...
5. In the main tarifia repo, go to `clients/apps/web` and install the local dependency with `pnpm` e.g. `pnpm add "/Users/<user>/Development/tarifia-js"` (or whatever your local path to the `tarifia-js` repository is).
