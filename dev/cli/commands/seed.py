...
def _configure_tarifia_self_integration() -> None:
    """Query the seeded admin org and configure Tarifia self-integration env vars."""
    result = run_command(
        ["uv", "run", "python", "-m", "scripts.seeds_load", "tarifia-self-env"],
        cwd=SERVER_DIR,
        capture=True,
    )
    ...
    console.print("[dim]Configured Tarifia self-integration in .env[/dim]")
...
def _print_seeded_login_info(new_org: str | None = None) -> None:
    ...
    login_info.add_row("Email", f"{new_org}@tarifia.sh" if new_org else "admin@tarifia.sh")
...
def register(app: typer.Typer, prompt_setup: callable) -> None:
    ...
    def seed(
        ...
        if reset:
            ...
            _configure_tarifia_self_integration()
