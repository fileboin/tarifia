...
SHARED_PROJECT_NAME = "tarifia-shared"
SHARED_NETWORK_NAME = "tarifia-shared"
...
# colliding.
REGISTRY_FILE = Path.home() / ".config" / "tarifia" / "docker-instances.json"
REGISTRY_VERSION = 1
MIN_INSTANCE = 1
MAX_INSTANCE = 99
...
def app_project(instance: int) -> str:
    return f"tarifia-app-{instance}"


def db_name(instance: int) -> str:
    return f"tarifia_dev_{instance}"
...
def s3_bucket(instance: int) -> str:
    return f"tarifia-s3-{instance}"


def s3_public_bucket(instance: int) -> str:
    return f"tarifia-s3-public-{instance}"
...
def _read_stored_instance() -> int | None:
    """Read TARIFIA_DOCKER_INSTANCE from .env.docker if set."""
    ...
    for line in ENV_FILE.read_text().splitlines():
        ...
        if line.startswith("#") or not line:
            continue
        match = re.match(r"^TARIFIA_DOCKER_INSTANCE\s*=\s*(\d+)\s*$", line)
        if match:
...
def _write_stored_instance(instance: int) -> None:
    """Write TARIFIA_DOCKER_INSTANCE to .env.docker."""
    _ensure_env_file()
    content = ENV_FILE.read_text()
    new_line = f"TARIFIA_DOCKER_INSTANCE={instance}"
    if re.search(r"^#?\s*TARIFIA_DOCKER_INSTANCE\s*=", content, re.MULTILINE):
        content = re.sub(
            r"^#?\s*TARIFIA_DOCKER_INSTANCE\s*=.*$",
            new_line,
            content,
            flags=re.MULTILINE,
        )
...
def _clear_stored_instance() -> bool:
    """Comment out TARIFIA_DOCKER_INSTANCE in .env.docker. Returns True if found."""
    if not ENV_FILE.exists():
        return False
    content = ENV_FILE.read_text()
    if re.search(r"^TARIFIA_DOCKER_INSTANCE\s*=", content, re.MULTILINE):
        content = re.sub(
            r"^TARIFIA_DOCKER_INSTANCE\s*=.*$",
            "# TARIFIA_DOCKER_INSTANCE=",
            content,
            flags=re.MULTILINE,
        )
        ENV_FILE.write_text(content)
...
def _detect_instance() -> tuple[int, str]:
    ...
    Priority:
    1. TARIFIA_DOCKER_INSTANCE in .env.docker (explicit per-worktree pin)
...
def _ensure_env_file() -> None:
    ...
    if not ENV_FILE.exists():
        if not ENV_TEMPLATE.exists():
            ...
            ENV_FILE.write_text(
                "# Tarifia Docker dev env\n"
                "# Set TARIFIA_DOCKER_INSTANCE=N to pin this worktree to instance N\n"
                "# TARIFIA_DOCKER_INSTANCE=\n"
            )
            return
...
def _ensure_server_env() -> None:
    ...
    Applies central secrets from ~/.config/tarifia/secrets.env when available,
...
def _ensure_network() -> None:
    """Idempotently create the tarifia-shared docker network."""
...
def _build_compose_env(instance: int) -> dict[str, str]:
    ...
    reached by container name on the tarifia-shared network — no host ports.
    """
    _assert_ports_free(instance)
    return {
        "TARIFIA_DOCKER_INSTANCE": str(instance),
        "API_PORT": str(api_port(instance)),
        "WEB_PORT": str(web_port(instance)),
        "TARIFIA_POSTGRES_DATABASE": db_name(instance),
        "TARIFIA_REDIS_DB": str(redis_db(instance)),
        "TARIFIA_S3_FILES_BUCKET_NAME": s3_bucket(instance),
        "TARIFIA_S3_FILES_PUBLIC_BUCKET_NAME": s3_public_bucket(instance),
    }
...
def _print_access_info(ctx: typer.Context, instance: int) -> None:
    ...
    console.print()
    console.print("[bold]Tarifia Docker Development Environment[/bold]")
    ...
    console.print(f"  psql:    dev docker exec db psql -U tarifia -d {db_name(instance)}")
...
def register(app: typer.Typer, prompt_setup: callable) -> None:
    ...
    def _route(service: str | None, instance: int) -> tuple[list[str], dict[str, str]]:
        ...
        - shared services (db, redis, minio, tinybird, prometheus, grafana) →
          the machine-wide `tarifia-shared` project
        - app services (api, worker, web) or no service → this instance's
          `tarifia-app-N` project
        """
    ...
    def docker_up(
        ...
        if not _shared_is_running():
            ...
            console.print("[bold blue]Starting Tarifia shared infra[/bold blue]")
        ...
        console.print(
            f"\n[bold blue]Starting Tarifia app stack (instance {instance})[/bold blue]\n"
        )
    ...
    def docker_exec(
        ...
        Examples:
          dev docker exec db psql -U tarifia -l
