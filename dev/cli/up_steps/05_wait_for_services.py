...
def wait_for_postgres(timeout: int = 60) -> bool:
    ...
    while time.time() - start_time < timeout:
        result = run_command(
            ["docker", "compose", "exec", "-T", "db", "pg_isready", "-U", "tarifia"],
            cwd=SERVER_DIR,
            capture=True,
        )
...
def run(ctx: Context) -> bool:
    ...
    if ctx.skip_tinybird:
        ...
        with step_spinner("Waiting for Tinybird..."):
            ...
            if token:
                _update_secrets_file("TARIFIA_TINYBIRD_API_TOKEN", token)
                _update_secrets_file("TARIFIA_TINYBIRD_READ_TOKEN", token)
                _update_secrets_file("TARIFIA_TINYBIRD_CLICKHOUSE_TOKEN", token)
