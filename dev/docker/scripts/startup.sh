#!/bin/bash
# Startup script for Tarifia API and Worker services in development mode
...
options = {'kid': 'tarifia_dev', 'use': 'sig'}
...
max_attempts=30
attempt=0
while ! pg_isready -h "$TARIFIA_POSTGRES_HOST" -p "$TARIFIA_POSTGRES_PORT" -U "$TARIFIA_POSTGRES_USER" -q; do
    attempt=$((attempt + 1))
...
BOOTSTRAP_MARKER="/app/server/.venv/.bootstrap-${TARIFIA_POSTGRES_DATABASE}.done"
if [[ "${1:-api}" == "api" && ! -f "$BOOTSTRAP_MARKER" ]]; then
    READ_USER="${TARIFIA_POSTGRES_READ_USER:-tarifia_read}"
    echo "Bootstrapping per-instance DB '$TARIFIA_POSTGRES_DATABASE' and MinIO buckets..."
    ...
    PGPASSWORD="$TARIFIA_POSTGRES_PWD" createdb -h "$TARIFIA_POSTGRES_HOST" \
        -p "$TARIFIA_POSTGRES_PORT" -U "$TARIFIA_POSTGRES_USER" \
        -O "$TARIFIA_POSTGRES_USER" "$TARIFIA_POSTGRES_DATABASE" 2>/dev/null || true
    PGPASSWORD="$TARIFIA_POSTGRES_PWD" psql -h "$TARIFIA_POSTGRES_HOST" \
        -p "$TARIFIA_POSTGRES_PORT" -U "$TARIFIA_POSTGRES_USER" \
        -d "$TARIFIA_POSTGRES_DATABASE" -v ON_ERROR_STOP=1 <<SQL >/dev/null
GRANT CONNECT ON DATABASE "$TARIFIA_POSTGRES_DATABASE" TO $READ_USER;
...
s3 = boto3.client(
    "s3",
    endpoint_url=os.environ["TARIFIA_S3_ENDPOINT_URL"],
    aws_access_key_id=os.environ["TARIFIA_AWS_ACCESS_KEY_ID"],
    aws_secret_access_key=os.environ["TARIFIA_AWS_SECRET_ACCESS_KEY"],
    config=Config(signature_version="s3v4"),
    region_name="us-east-1",
)
private = os.environ["TARIFIA_S3_FILES_BUCKET_NAME"]
public = os.environ["TARIFIA_S3_FILES_PUBLIC_BUCKET_NAME"]
...
PY
    ...
    echo "Bootstrap complete: $TARIFIA_POSTGRES_DATABASE, $TARIFIA_S3_FILES_BUCKET_NAME, $TARIFIA_S3_FILES_PUBLIC_BUCKET_NAME"
fi
...
# may be stale.
if [[ -n "${TARIFIA_TINYBIRD_CLICKHOUSE_URL:-}" ]]; then
    TINYBIRD_HOST=$(echo "$TARIFIA_TINYBIRD_CLICKHOUSE_URL" | sed 's|http://||' | cut -d: -f1)
    ...
    if [[ -n "${TB_TOKEN:-}" ]]; then
        export TARIFIA_TINYBIRD_API_TOKEN="$TB_TOKEN"
        export TARIFIA_TINYBIRD_CLICKHOUSE_TOKEN="$TB_TOKEN"
        export TARIFIA_TINYBIRD_READ_TOKEN="$TB_TOKEN"
...
case "${1:-api}" in
    api)
        ...
        exec uv run uvicorn tarifia.app:app \
            --reload \
            --reload-dir tarifia \
            --host 0.0.0.0 \
            --port 8000 \
