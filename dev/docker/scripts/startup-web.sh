#!/bin/bash
# Startup script for Tarifia Web frontend in development mode
...
echo "=== Tarifia Web Frontend Startup ==="
...
if [[ ! -d "packages/client/dist" ]]; then
    echo "Building @tarifia-sh/client..."
    pnpm --filter @tarifia-sh/client build
else
    echo "@tarifia-sh/client already built"
fi

if [[ ! -d "packages/ui/dist" ]]; then
    echo "Building @tarifia-sh/ui..."
    pnpm --filter @tarifia-sh/ui build
else
    echo "@tarifia-sh/ui already built"
fi

if [[ ! -d "packages/orbit/dist" ]]; then
    echo "Building @tarifia-sh/orbit..."
    pnpm --filter @tarifia-sh/orbit build
else
    echo "@tarifia-sh/ui already built"
fi

if [[ ! -d "packages/checkout/dist" ]]; then
    echo "Building @tarifia-sh/checkout..."
    pnpm --filter @tarifia-sh/checkout build
else
    echo "@tarifia-sh/checkout already built"
fi
