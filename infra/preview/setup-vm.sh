...
TARIFIA_PREVIEW_ACCESS_TOKEN=${PREVIEW_ACCESS_TOKEN}
VERCEL_BYPASS_SECRET=${VERCEL_BYPASS_SECRET}
ENVFILE
    cat > /etc/preview-secrets/stripe.env <<ENVFILE
TARIFIA_STRIPE_SECRET_KEY=${STRIPE_SECRET_KEY}
TARIFIA_STRIPE_WEBHOOK_SECRET=${STRIPE_WEBHOOK_SECRET}
ENVFILE
    cat > /etc/preview-secrets/pydantic_ai.env <<ENVFILE
TARIFIA_PYDANTIC_AI_GATEWAY_API_KEY=${PYDANTIC_AI_GATEWAY_API_KEY}
ENVFILE
    ...
    echo "Add to GitHub secrets: TARIFIA_PREVIEW_ACCESS_TOKEN"
    echo "Add to Vercel env vars: TARIFIA_PREVIEW_ACCESS_TOKEN"
    echo ""
else
...
:8080 {
	route {
		@no_token not header X-Preview-Token {$TARIFIA_PREVIEW_ACCESS_TOKEN}
...
if [[ ! -f "$KEY_FILE" ]]; then
    ssh-keygen -t ed25519 -f "$KEY_FILE" -N "" -C "tarifia-preview-deploy"
...
rm -f /etc/sudoers.d/tarifia-preview
...
for f in tarifia-preview-backend@.service tarifia-preview-frontend@.service tarifia-preview-logs.service tarifia-preview-infra.path tarifia-preview-infra.service tarifia-preview-hibernate.service tarifia-preview-hibernate.timer; do
    if [[ -f "${SCRIPT_DIR}/${f}" ]]; then
        cp "${SCRIPT_DIR}/${f}" "/etc/systemd/system/${f}"
    fi
done
systemctl daemon-reload
systemctl enable tarifia-preview-logs tarifia-preview-infra.path tarifia-preview-hibernate.timer
systemctl start tarifia-preview-logs 2>/dev/null || true
systemctl start tarifia-preview-infra.path
systemctl start tarifia-preview-hibernate.timer
...
echo "  TARIFIA_PREVIEW_HOST = $(tailscale ip -4)"
echo "  TARIFIA_PREVIEW_SSH_KEY = (contents below)"
echo ""
echo "--- Private key (add as TARIFIA_PREVIEW_SSH_KEY) ---"
cat "$KEY_FILE"
echo ""
...
echo "  TARIFIA_PREVIEW_USER = ${DEPLOY_USER}"
echo ""
