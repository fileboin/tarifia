...
Comprehensive load testing suite for Tarifia's payment infrastructure, focusing on checkout flows, webhook processing, and background job performance.
...
- Python 3.14+ with uv
- Running Tarifia API server (or staging environment)
...
```bash
LOAD_TEST_API_TOKEN=tarifia_oat_...                           # Organization access token (required)
...
LOAD_TEST_API_TOKEN="tarifia_oat_..." \
uv run task loadtest
...
```bash
LOAD_TEST_HOST=https://staging.tarifia.sh
LOAD_TEST_API_TOKEN=tarifia_pat_...          # Personal access token
