...
from tarifia.config import Environment, settings
from tarifia.email.schemas import OAuth2LeakedTokenEmail, OAuth2LeakedTokenProps
...
from tarifia.models import OAuth2Token, User
from tarifia.oauth2.repository import OAuth2TokenRepository
...
# TEMPORARY: the Tarifia app (iOS/Android/web, @tarifia-sh/app) does not refresh
...
_APP_CLIENT_IDS: dict[Environment, str] = {
    Environment.production: "tarifia_ci_yZLBGwoWZVsOdfN5CODRwVSTlJfwJhXqwg65e2CuNMZ",
    Environment.development: "tarifia_ci_hbFdMZZRghgdm2F4LMceQSrcQNunmjlh6ukGJ1dG0Vg",
}
    ...
    async def get_by_access_token(
        ...
        if cast(bool, token.is_expired()):
            if token.client_id != APP_CLIENT_ID:
                return None
            log.warning(
                "Allowing expired access token from Tarifia app client",
                token_id=token.id,
        ...
        *,
        ...
        for recipient in recipients:
            enqueue_email_template(
                ...
                subject="Security Notice - Your Tarifia Access Token has been leaked",
            )
