...
from tarifia.models import Account, Order, Payout
from tarifia.models.transaction import PlatformFeeType
from tarifia.postgres import AsyncSession
from tarifia.transaction.repository import TransactionRepository
...
class InvoiceError(TarifiaError): ...
...
class InvoiceService:
    ...
    async def create_payout_invoice(self, session: AsyncSession, payout: Payout) -> str:
        ...
        items = [
            InvoiceItem(
                description=f"Digital services and products resold by Tarifia.sh\nFrom {earliest.strftime('%Y-%m-%d')} to {latest.strftime('%Y-%m-%d')}",
                quantity=1,
                unit_amount=gross_amount,
                amount=gross_amount,
            ),
            InvoiceItem(
                description="Tarifia revenue share",
                quantity=1,
        ...
        invoice = Invoice(
            ...
            + (
                "Tarifia Software, Inc. is the merchant of record reselling digital services.\n"
                "Tarifia Software, Inc. captures and remits international sales tax from such sales – as needed.\n"
