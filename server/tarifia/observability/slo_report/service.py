...
from tarifia.observability.slo import CRITICAL_ENDPOINTS
...
100 * (1 - (
  (sum(increase(tarifia_http_request_total{env="$env", status_code=~"5.."}[$range])) or vector(0))
  / sum(increase(tarifia_http_request_total{env="$env"}[$range]))
))
"""
...
100 * clamp_min(1 - (
  (sum(increase(tarifia_http_request_total{env="$env", status_code=~"5.."}[$range])) or vector(0))
  / clamp_min(0.0005 * sum(increase(tarifia_http_request_total{env="$env"}[$range])), 0.001)
), 0)
"""

ENDPOINT_P99_QUERY = """
histogram_quantile(0.99, sum by (endpoint, method, le) (
  rate(tarifia_http_request_duration_seconds_bucket{env="$env"}[$range])
))
"""

ENDPOINT_AVAILABILITY_QUERY = """
100 * (1 - (
  sum by (endpoint, method) (increase(tarifia_http_request_total{env="$env", status_code=~"5.."}[$range]))
  or (sum by (endpoint, method) (increase(tarifia_http_request_total{env="$env"}[$range])) * 0)
) / clamp_min(sum by (endpoint, method) (increase(tarifia_http_request_total{env="$env"}[$range])), 1))
"""

ENDPOINT_REQUEST_COUNT_QUERY = """
sum by (endpoint, method) (increase(tarifia_http_request_total{env="$env"}[$range]))
"""
...
TOTAL_ERRORS_QUERY = """
sum(increase(tarifia_http_request_total{env="$env", status_code=~"5.."}[$range]))
"""
