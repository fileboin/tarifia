...
from tarifia import worker  # noqa
from tarifia.api import router
from tarifia.auth.exception_handlers import (
    TarifiaAuthRedirectionError,
    auth_redirection_error_exception_handler,
)
from tarifia.auth.middlewares import AuthSubjectMiddleware
from tarifia.backoffice import app as backoffice_app
from tarifia.checkout import ip_geolocation
from tarifia.checkout_link.app import app as checkout_link_redirect_app
from tarifia.config import settings
from tarifia.exception_handlers import add_exception_handlers
from tarifia.health.endpoints import router as health_router
from tarifia.kit.cors import CORSConfig, CORSMatcherMiddleware, Scope
...
from tarifia.middlewares import (
...
from tarifia.observability.remote_write import (
    start_remote_write_pusher,
    stop_remote_write_pusher,
)
from tarifia.observability.slo import start_slo_metrics, stop_slo_metrics
from tarifia.openapi import OPENAPI_PARAMETERS, APITag, set_openapi_generator
...
from tarifia.search.endpoints import router as search_router
from tarifia.sentry import configure_sentry
from tarifia.version import CURRENT_API_VERSION, APIVersionMiddleware
from tarifia.webhook.webhooks import document_webhooks
...
def configure_cors(app: FastAPI) -> None:
    ...
    # Tarifia frontend CORS configuration
    if settings.CORS_ORIGINS:

        def tarifia_frontend_matcher(origin: str, scope: Scope) -> bool:
            return origin in settings.CORS_ORIGINS

        tarifia_frontend_config = CORSConfig(
            tarifia_frontend_matcher,
            allow_origins=[str(origin) for origin in settings.CORS_ORIGINS],
            allow_credentials=True,  # Cookies are allowed, but only there!
            allow_methods=["*"],
            allow_headers=["*"],
        )
        configs.append(tarifia_frontend_config)
...
def create_app() -> FastAPI:
    ...
    app.add_exception_handler(
        TarifiaAuthRedirectionError, auth_redirection_error_exception_handler
    )
