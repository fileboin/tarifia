...
from tarifia.observability import OPERATIONAL_ERROR_TOTAL
...
def _external_event_already_handled_error_matcher(exc: BaseException) -> bool:
    # Import deferred to avoid circular dependency with tarifia.worker
    from tarifia.external_event.service import ExternalEventAlreadyHandled
...
def _tinybird_operational_error_matcher(exc: BaseException) -> bool:
    # Import deferred to avoid circular dependency with tarifia.worker
    from tarifia.integrations.tinybird.client import TinybirdOperationalError

    return isinstance(exc, TinybirdOperationalError)


def _tarifia_self_client_operational_error_matcher(exc: BaseException) -> bool:
    # Import deferred to avoid circular dependency with tarifia.worker
    from tarifia.integrations.tarifia.client import TarifiaSelfClientOperationalError

    return isinstance(exc, TarifiaSelfClientOperationalError)


def _email_sender_operational_error_matcher(exc: BaseException) -> bool:
    # Import deferred to avoid circular dependency
    from tarifia.email.sender import EmailSenderOperationalError
...
_operation_error_matchers: dict[str, OperationalErrorMatcher] = {
    ...
    "tarifia_self_client_operational_error": _tarifia_self_client_operational_error_matcher,
