...
from tarifia.logging import CorrelationID, Logger
...
_job_queue_manager: contextvars.ContextVar["JobQueueManager | None"] = (
    contextvars.ContextVar("tarifia.job_queue_manager")
)
...
class JobQueueManager:
    ...
    def enqueue_job(
        ...
        **kwargs: JSONSerializable,
        ...
        log.debug("tarifia.worker.job_enqueued", actor=actor, delay=delay)
    ...
    async def flush(self, broker: dramatiq.Broker, redis: Redis) -> None:
        ...
        for actor_name, encoded_message in all_messages:
            log.debug(
                "tarifia.worker.job_flushed", actor=actor_name, message=encoded_message
            )
