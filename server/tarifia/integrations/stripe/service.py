...
from tarifia.logfire import instrument_httpx
...
#: :class:`tarifia.models.payment.PaymentTrigger`. Producers (in
#: ``tarifia.order.service``) and the consumer in ``tarifia.integrations.stripe.payment``
...
class StripeError(TarifiaError): ...
...
class StripeService:
    ...
    async def reverse_transfer(
        ...
        *,
        ...
        reverse_params: TransferCreateReversalParams = {
            "metadata": metadata or {},
            "idempotency_key": f"tarifia:transfer_reverse:{id}",
        }
    ...
    async def create_tax_transaction(
        ...
        return await stripe_lib.tax.Transaction.create_from_calculation_async(
            ...
            reference=reference,
            idempotency_key=f"tarifia:tax_transaction:{reference}",
        )
    ...
    async def revert_tax_transaction(
        ...
        params: TransactionCreateReversalParams = {
            ...
            "reference": reference,
            "idempotency_key": f"tarifia:tax_transaction_revert:{reference}",
        }
