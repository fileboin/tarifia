...
from tarifia.account.repository import AccountRepository
from tarifia.config import settings
from tarifia.email.schemas import (
    EmailAdapter,
    TarifiaSelfSubscriptionConfirmationProps,
    TarifiaSelfSubscriptionCycledProps,
)
from tarifia.email.sender import Attachment, enqueue_email_template
...
class TarifiaSelfService:
    ...
    def is_configured(self) -> bool:
        return settings.TARIFIA_SELF_ENABLED
    ...
    def enqueue_update_customer_slug(
        ...
        if not self.is_configured:
            return
        enqueue_job(
            "tarifia_self.update_customer_slug",
            external_id=str(organization_id),
            slug=slug,
        )
    ...
    def enqueue_track_organization_review_usage(
        ...
        *,
        ...
        if not self.is_configured:
            return
        if external_customer_id == settings.TARIFIA_ORGANIZATION_ID:
            return
        if cost_usd is None:
            return
    ...
    async def list_plans(self) -> list["Product"]:
        if not self.is_configured:
            raise TarifiaSelfNotConfigured()
    ...
    async def change_plan(
        ...
        *,
        ...
        if subscription is None:
            raise TarifiaSelfNoActiveSubscription(organization_id)
        ...
        if product_id == settings.TARIFIA_SCALE_PRODUCT_ID:
            discount_id = await startup_program_service.resolve_checkout_discount_id(
                organization_id=organization_id, product_id=product_id
            )
    ...
    async def claim_startup_program(
        ...
        *,
        ...
        - **Free → Scale**: returns ``(None, Checkout)`` — the org needs to
          complete a Tarifia checkout to set up a payment method. The discount
        ...
        discount_id = await startup_program_service.resolve_checkout_discount_id(
            ...
            product_id=settings.TARIFIA_SCALE_PRODUCT_ID,
        )
        ...
        needs_switch = subscription.product_id != settings.TARIFIA_SCALE_PRODUCT_ID
    ...
    async def get_billing_details(
        ...
        *,
        ...
        await self._ensure_tarifia_customer(organization_id)
    ...
    async def create_customer_session(
        ...
        *,
        ...
        """Create a customer session token for the org's Tarifia billing customer.
    ...
    async def _ensure_tarifia_customer(self, organization_id: uuid.UUID) -> None:
        if not self.is_configured:
            raise TarifiaSelfNotConfigured()
        ...
        if customer is None:
            raise TarifiaSelfCustomerNotFound(organization_id)
    ...
    async def get_order_invoice_url(
        ...
        if customer is None:
            raise TarifiaSelfOrderNotFound(order_id)
    ...
    async def handle_benefit_grant_event(
        ...
        with logfire.span(
            "tarifia_self.webhook.benefit_grant",
    ...
    async def handle_order_created_event(
        ...
        product_name = order.product.name if order.product is not None else "Tarifia"

        if order.billing_reason == OrderBillingReason.SUBSCRIPTION_CYCLE:
            template_name = "tarifia_self_subscription_cycled"
            subject = f"Your {product_name} subscription renewed"
        else:
            template_name = "tarifia_self_subscription_confirmation"
        ...
        if invoice_expected:
            if not order.is_invoice_generated:
                ...
                # retry — generation runs asynchronously on Tarifia's side.
                try:
                    await client.trigger_order_invoice_generation(order_id=order.id)
                except TarifiaSelfNotPaidOrder as e:
                    raise TarifiaSelfInvoiceNotReady(order.id) from e
                raise TarifiaSelfInvoiceNotReady(order.id)
        ...
        with logfire.span(
            "tarifia_self.webhook.order_created",
            order_id=order.id,
            billing_reason=order.billing_reason.value,
        ):
            for recipient in recipients:
                if template_name == "tarifia_self_subscription_confirmation":
                    email = EmailAdapter.validate_python(
                        {
                            "template": template_name,
                            "props": TarifiaSelfSubscriptionConfirmationProps(
                                email=recipient,
                    ...
                    email = EmailAdapter.validate_python(
                        {
                            "template": template_name,
                            "props": TarifiaSelfSubscriptionCycledProps(
                                email=recipient,
                                product_name=product_name,
                            ).model_dump(),
                        }
    ...
    async def _require_approval(
        ...
        *,
        ...
        if organization is None or not organization.is_active():
            raise TarifiaSelfNotApproved(organization_id)
    ...
    async def _ensure_plan(self, product_id: str) -> "Product":
        ...
        if plan is None:
            raise TarifiaSelfPlanNotFound(product_id)
        return plan
    ...
    def _resolve_organization_id(
        ...
        if not isinstance(raw, str):
            raise TarifiaSelfWebhookError(f"Customer {customer.id} has no external_id")
        try:
            return uuid.UUID(raw)
        except ValueError as e:
            raise TarifiaSelfWebhookError(
    ...
    async def _apply_transaction_fee(
        ...
        with logfire.span(
            "tarifia_self.webhook.transaction_fee.applied",
            organization_id=str(organization_id),
    ...
    async def _apply_support(
        ...
        with logfire.span(
            "tarifia_self.webhook.support.applied",
            organization_id=str(organization_id),
            level=level,
            slack=slack,
    ...
    async def _apply_preview_access(
        ...
        with logfire.span(
            "tarifia_self.webhook.preview_access.applied",
            organization_id=str(organization_id),
            enabled=enabled,
        ):
...
tarifia_self = TarifiaSelfService()
