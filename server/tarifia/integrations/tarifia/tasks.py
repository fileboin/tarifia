...
from tarifia.integrations.tinybird.service import count_user_events_by_organization
...
from .exceptions import TarifiaSelfInvoiceNotReady
from .service import tarifia_self


@actor(actor_name="tarifia_self.create_customer", priority=TaskPriority.LOW)
...
@actor(actor_name="tarifia_self.add_member", priority=TaskPriority.LOW)
...
@actor(actor_name="tarifia_self.update_member", priority=TaskPriority.LOW)
...
@actor(actor_name="tarifia_self.update_customer_slug", priority=TaskPriority.LOW)
...
@actor(actor_name="tarifia_self.remove_member", priority=TaskPriority.LOW)
...
@actor(actor_name="tarifia_self.delete_customer", priority=TaskPriority.LOW)
async def delete_customer(external_id: str) -> None:
    await get_client().delete_customer(external_id=external_id)


@actor(
    actor_name="tarifia_self.track_event_ingestion_v2",
...
async def track_event_ingestion() -> None:
    if not settings.TARIFIA_SELF_ENABLED:
        return
    self_organization_id = uuid.UUID(settings.TARIFIA_ORGANIZATION_ID)
    cutoff = utc_now() - timedelta(seconds=30)
    async with AsyncSessionMaker() as session:
        repository = EventRepository.from_session(session)
        last_flush = await repository.get_latest_tarifia_self_ingestion_timestamp(
...
@actor(
    actor_name="tarifia_self.track_organization_review_usage",
    priority=TaskPriority.LOW,
)
...
async def webhook_benefit_grant_created(event_id: uuid.UUID) -> None:
    async with AsyncSessionMaker() as session:
        async with external_event_service.handle(
            session, ExternalEventSource.tarifia, event_id
        ) as event:
...
@actor(actor_name="tarifia_self.webhook.order.created", priority=TaskPriority.LOW)
async def webhook_order_created(event_id: uuid.UUID) -> None:
    async with AsyncSessionMaker() as session:
        async with external_event_service.handle(
            ...
            except TarifiaSelfInvoiceNotReady as e:
                if can_retry():
                    raise Retry() from e
                raise
