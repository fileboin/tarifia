...
from tarifia.kit.utils import human_readable_size
...
class S3File(IDSchema, validate_assignment=True):
    ...
    def to_metadata(self) -> dict[str, str]:
        metadata = {
            "tarifia-id": str(self.id),
            "tarifia-organization-id": str(self.organization_id),
            "tarifia-name": self.name.encode("ascii", "ignore").decode("ascii"),
            "tarifia-size": str(self.size),
        }
        if self.checksum_sha256_base64:
            metadata["tarifia-sha256-base64"] = self.checksum_sha256_base64
        if self.checksum_sha256_hex:
            metadata["tarifia-sha256-hex"] = self.checksum_sha256_hex
        return metadata
    ...
    def from_head(cls, path: str, head: dict[str, Any]) -> Self:
        ...
        return cls(
            id=metadata.get("tarifia-id"),
            organization_id=metadata.get("tarifia-organization-id"),
            name=metadata.get("tarifia-name"),
            path=path,
            mime_type=head["ContentType"],
            size=metadata.get("tarifia-size"),
            storage_version=head.get("VersionId", None),
            checksum_etag=head.get("ETag", None),
            checksum_sha256_base64=metadata.get("tarifia-sha256-base64"),
            checksum_sha256_hex=metadata.get("tarifia-sha256-hex"),
