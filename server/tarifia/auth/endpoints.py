...
from tarifia.auth.exceptions import (
    TarifiaAuthError,
    TarifiaAuthRedirectionError,
    UnavailableFactorError,
)
from tarifia.auth.oauth2.github import get_github_factor
from tarifia.auth.oauth2.google import get_google_factor
from tarifia.authz.dependencies import AuthorizeWebUserRead, AuthorizeWebUserWrite
from tarifia.exceptions import NotPermitted, ResourceNotFound
from tarifia.models import UserSession as UserSession
...
async def complete(
    ...
    if user is None:
        raise TarifiaAuthRedirectionError("User not found for authenticated identity")
...
async def email_otp_verify(
    ...
    try:
        ...
        raise TarifiaAuthError("Invalid or expired OTP", 403) from e
...
async def totp_enroll(
    ...
    try:
        ...
        raise TarifiaAuthError("TOTP factor already enrolled", 409) from e
    ...
    return TOTPEnrollment(
        ...
        provisioning_uri=enrollment.get_provisioning_uri(user.email, "Tarifia"),
    )
...
async def totp_enable(
    ...
    try:
        ...
        raise TarifiaAuthError("TOTP factor not enrolled", 403) from e
    except AlreadyEnabledTOTPException as e:
        raise TarifiaAuthError("TOTP factor already enabled", 409) from e
    except InvalidTOTPCodeException as e:
        raise TarifiaAuthError("Invalid TOTP code", 403) from e
    return None
...
async def backup_codes_verify(
    ...
    try:
        ...
        raise TarifiaAuthError("Invalid or expired backup code", 400) from e
