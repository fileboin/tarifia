...
from tarifia.kit.http import ReturnTo, is_localhost
...
from ..exceptions import GetEmailError, TarifiaAuthRedirectionError
...
def _check_factor(
    ...
    for available_factor in available_factors:
        if available_factor.identifier == factor.identifier:
            return
    raise TarifiaAuthRedirectionError("Factor not available for this session")
    ...
    *,
    ...
    async def _authorize(
        ...
        try:
            ...
            raise TarifiaAuthRedirectionError(OIDC_ERROR_MESSAGE) from e
    ...
    async def _callback(
        ...
        if state is None:
            raise TarifiaAuthRedirectionError("Missing OAuth2 state")
        ...
        if request.method != "POST":
            ...
            if state_cookie is None:
                raise TarifiaAuthRedirectionError("Missing OAuth2 state cookie")

            if state != state_cookie:
                raise TarifiaAuthRedirectionError("Invalid OAuth2 state")
        ...
        if authentication_session is None:
            ...
            if context is None:
                raise TarifiaAuthRedirectionError("No active authentication session")
        ...
        # New user
            ...
            try:
                ...
                raise TarifiaAuthRedirectionError(e.message) from e
