...
from tarifia.kit.db.models import RateLimitGroupMixin, RecordModel
from tarifia.kit.extensions.sqlalchemy import StringEnum

from .account import Account

if TYPE_CHECKING:
    from tarifia.email.sender import EmailFromReply
...
class PayoutAccountNotReady(TarifiaError):
...
class InvalidStatusTransitionError(TarifiaError):
    def __init__(
...
class Organization(RateLimitGroupMixin, RecordModel):
    ...
    # from the active Tarifia support grant (see SupportTier). NULL = free; only
