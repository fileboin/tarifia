...
from tarifia.enums import InvoiceNumbering, SubscriptionProrationBehavior
...
from tarifia.kit.repository import Options
...
from tarifia.organization_access_token.repository import (
...
from tarifia.payout_account.service import payout_account as payout_account_service
from tarifia.postgres import AsyncReadSession, AsyncSession, sql
...
from tarifia.webhook.repository import WebhookEndpointRepository
...
# its own — Tarifia grants it automatically (downloadables, license_keys,
...
class OrganizationError(TarifiaError): ...
...
class OrganizationService:
    ...
    async def create(
        ...
        tarifia_self_service.enqueue_create_customer(
        ...
        await self.add_user(
            ...
            enqueue_tarifia_self_member=False,
        )
    ...
    async def soft_delete_organization(
        ...
        *,
        ...
        if organization._avatar_url:
            # Anonymize by setting to Tarifia logo
        ...
        tarifia_self_service.enqueue_delete_customer(organization_id=organization.id)
    ...
    async def add_user(
        ...
        *,
        ...
        tarifia_self_member_delay: int | None = None,
        enqueue_tarifia_self_member: bool = True,
    ) -> None:
        nested = await session.begin_nested()
        try:
    ...
    async def change_owner(
        ...
        *,
        ...
        await self._sync_tarifia_self_customer_owner(
            session,
            organization_id=organization_id,
            new_owner_user=new_owner_user,
        )

    async def _sync_tarifia_self_customer_owner(
        self,
        session: AsyncSession,
        *,
        organization_id: uuid.UUID,
        ...
        tarifia_organization_id = uuid.UUID(settings.TARIFIA_ORGANIZATION_ID)
        if organization_id == tarifia_organization_id:
            return

        customer_repository = CustomerRepository.from_session(session)
        customer = await customer_repository.get_by_external_id_and_organization(
            str(organization_id), tarifia_organization_id
        )
        if customer is None:
            raise CannotChangeOwnerError(
                f"Tarifia self customer not found for organization {organization_id}"
            )
        ...
        if target_member is None:
            raise CannotChangeOwnerError(
                f"Tarifia self member not found for user {new_owner_user.id}"
            )
    ...
    async def _build_setup_readiness_check(
        ...
        auto-fulfillable checkout link (selling a Tarifia-fulfilled benefit,
    ...
    async def enable_preview_access(
        ...
        (`TarifiaSelfService._apply_preview_access`) by flipping on every
        ...
        organization = await repository.update(
            ...
            update_dict={
                "feature_settings": {
                    ...
                    **{
                        flag: True
                        for flag in tarifia_self_service.PREVIEW_ACCESS_FEATURE_FLAGS
                    },
                }
            },
        )
