...
from tarifia.account.schemas import Account as AccountSchema
from tarifia.account.service import account as account_service
from tarifia.auth.permission import ROLE_PERMISSIONS
from tarifia.authz.dependencies import (
...
from tarifia.email.schemas import OrganizationInviteEmail, OrganizationInviteProps
...
from tarifia.integrations.tarifia.schemas import (
...
from tarifia.models import Account, Organization, UserOrganization
...
from tarifia.payout_account.repository import PayoutAccountRepository
...
from tarifia.support_case.schemas import (
    HumanReviewRequest,
    SupportCaseNotFound,
)
from tarifia.support_case.schemas import (
...
from tarifia.user_organization.schemas import (
    OrganizationMember,
...
async def invite_member(
    ...
    enqueue_email_template(
        ...
        to_email_addr=email,
        subject=f"You've been invited to {organization.name} on Tarifia",
    )
...
async def list_plans(
    ...
    """
    products = await tarifia_self_service.list_plans()
    subscription = await tarifia_self_service.get_subscription(authz.organization.id)
    free_plan = await tarifia_self_service.resolve_free_plan(
    ...
    *,
    ...
    if settings.STARTUP_PROGRAM_ENABLED:
        subscription_obj.startup_program_scale_product_id = (
            settings.TARIFIA_SCALE_PRODUCT_ID
        )
...
async def get_subscription(
    ...
    """Get the current Tarifia subscription for this organization.
...
async def start_subscription_checkout(
    ...
    """Create a Tarifia checkout session for an initial paid subscription."""
    customer_ip_address = request.client.host if request.client else None
    checkout = await tarifia_self_service.start_checkout(
        session=session,
        organization_id=authz.organization.id,
...
async def change_subscription_plan(
    ...
    subscription = await tarifia_self_service.change_plan(
        session=session,
        organization_id=authz.organization.id,
        product_id=body.product_id,
    )
...
async def cancel_subscription_endpoint(
    ...
    """
    subscription = await tarifia_self_service.cancel_subscription(
        organization_id=authz.organization.id,
    )
    return await _enrich_subscription(
        OrganizationSubscription.from_sdk(subscription),
...
async def claim_startup_program(
    ...
    customer_ip_address = request.client.host if request.client else None
    try:
        subscription, checkout = await tarifia_self_service.claim_startup_program(
            session=session,
            organization_id=authz.organization.id,
            customer_ip_address=customer_ip_address,
            success_url=body.success_url,
...
async def list_orders(
    ...
    """List Tarifia orders billed to this organization."""
    items, total = await tarifia_self_service.list_orders(
...
async def get_billing_details(
    ...
    """Get the billing name, address, and tax ID used on Tarifia invoices."""
    customer = await tarifia_self_service.get_billing_details(
        authz.organization.id,
...
async def update_billing_details(
    ...
    """Update the billing name, address, and tax ID used on Tarifia invoices."""
    customer = await tarifia_self_service.update_billing_details(
        authz.organization.id,
        update=body,
        external_member_id=str(authz.auth_subject.subject.id),
    )
...
async def create_customer_session(
    ...
    """Create a customer session token bound to this org's Tarifia billing
    customer. The returned token authenticates against
    `/v1/customer-portal/customers/me/*` for the duration of its TTL."""
    token = await tarifia_self_service.create_customer_session(
        authz.organization.id,
        external_member_id=str(authz.auth_subject.subject.id),
    )
...
@router.delete(
    ...
    responses={
        ...
        400: {
            ...
            "model": TarifiaSelfPaymentMethodInUse.schema(),
        },
        404: {
...
async def delete_payment_method(
    ...
    """Delete a saved payment method used to pay Tarifia invoices."""
    await tarifia_self_service.delete_payment_method(
        authz.organization.id,
...
async def set_default_payment_method(
    ...
    """Set the default payment method used to pay Tarifia invoices."""
    await tarifia_self_service.set_default_payment_method(
        authz.organization.id,
        payment_method_id=payment_method_id,
        external_member_id=str(authz.auth_subject.subject.id),
    )
...
async def get_order_invoice(
    ...
    """Get the invoice URL for a Tarifia order belonging to this organization."""
    url = await tarifia_self_service.get_order_invoice_url(
...
async def list_benefit_grants(
    ...
    """List Slack shared channel benefit grants attached to this org's Tarifia
    subscription."""
    grants = await tarifia_self_service.list_benefit_grants(
        authz.organization.id,
        external_member_id=str(authz.auth_subject.subject.id),
    )
...
async def update_benefit_grant(
    ...
    grant = await tarifia_self_service.update_benefit_grant(
        authz.organization.id,
