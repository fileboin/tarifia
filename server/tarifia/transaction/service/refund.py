...
from tarifia.event.system import BalanceRefundMetadata, SystemEvent, build_system_event
...
from tarifia.models import Customer, Order, Refund, Transaction
...
class RefundTransactionService(BaseTransactionService):
    async def create(self, session: AsyncSession, refund: Refund) -> Transaction:
        ...
        if refund.processor == PaymentProcessor.stripe:
            ...
            settlement_tax_amount = -tarifia_round(refund.tax_amount * exchange_rate)
        else:
