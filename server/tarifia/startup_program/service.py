...
from tarifia.integrations.tarifia.client import get_client
from tarifia.models import Organization

if TYPE_CHECKING:
    from tarifia_sdk.models import Discount
...
class StartupProgramError(TarifiaError): ...


class StartupProgramService:
    """Startup Program logic, driven entirely through the Tarifia API.

    All reads and writes against billing-side entities (the Tarifia-for-Tarifia
    ...
    async def mark_invited(self, organization: Organization) -> "Discount":
        ...
        Tarifia-for-Tarifia customer's metadata. Idempotent: if the customer
        ...
        if not settings.STARTUP_PROGRAM_ENABLED:
            raise StartupProgramError(
                ...
                "(TARIFIA_ORGANIZATION_ID / TARIFIA_SCALE_PRODUCT_ID / TARIFIA_ACCESS_TOKEN)."
            )
    ...
    async def _mark_invited_inner(self, organization: Organization) -> "Discount":
        from tarifia_sdk.models import DiscountDuration
        ...
        if customer is None:
            raise StartupProgramError(
                "No Tarifia customer for organization "
        ...
        # ``organization_id`` is intentionally omitted: ``TARIFIA_ACCESS_TOKEN``
    ...
    async def uninvite(self, organization: Organization) -> None:
        ...
        if not settings.STARTUP_PROGRAM_ENABLED:
            raise StartupProgramError(
                "Startup Program is not configured "
                "(TARIFIA_ORGANIZATION_ID / TARIFIA_SCALE_PRODUCT_ID)."
            )
    ...
    async def get_status(self, organization_id: uuid.UUID) -> str | None:
        ...
        ``None`` when the feature is disabled, the org has no Tarifia customer,
    ...
    async def resolve_checkout_discount_id(
        ...
        """Return the discount id to attach to a Tarifia-for-Tarifia checkout.
    ...
    async def _get_customer_discount(
        ...
        """Resolve the customer's Startup Program discount via the Tarifia API.
    ...
    async def _send_welcome_email(
        ...
        Recipients come from the Tarifia API's billing-contacts endpoint
        ...
        for recipient in recipients:
            email = TarifiaSelfStartupProgramWelcomeEmail(
            ...
            enqueue_email_template(
                ...
                subject="Welcome to the Tarifia Startup Program",
            )
