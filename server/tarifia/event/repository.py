...
from tarifia.kit.repository.base import Options
...
from tarifia.models.product_price import ProductPriceMeteredUnit
...
class EventRepository(RepositoryBase[Event], RepositoryIDMixin[Event, UUID]):
    ...
    async def resolve_pending_parents(
        ...
        row. Run as four UPDATEs to keep each plan cleanly indexed:
          ...
          1c — child in batch, parent in DB, ref by Tarifia ID (UUID)
          1b — parent in batch, child pending in DB, ref by external_id
          1d — parent in batch, child pending in DB, ref by Tarifia ID (UUID)
    ...
    async def get_latest_tarifia_self_ingestion_timestamp(
