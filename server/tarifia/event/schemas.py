...
from tarifia.kit.metadata import METADATA_DESCRIPTION, MetadataValue
...
_SOURCE_DESCRIPTION = (
    ...
    "`system` events are created by Tarifia. "
...
class EventCreateBase(Schema):
    ...
    parent_id: str | None = Field(
        ...
        description=(
            ...
            "Can be either a Tarifia event ID (UUID) or an external event ID."
        ),
    )
    metadata: EventMetadataInput = Field(
        description=METADATA_DESCRIPTION.format(
            heading=(
                ...
                "Some keys like `_llm` are structured data that are handled specially by Tarifia."
            )
        ),
...
class CustomerStat(Schema):
    ...
    customer_id: UUID4 | None = Field(description="Tarifia customer ID.")
