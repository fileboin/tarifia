...
from tarifia.logging import ClientContext, CorrelationID, Logger
from tarifia.operational_errors import handle_operational_error
...
class LogCorrelationIdMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        client_context = {
            key: value
            for key, value in (
                ("client_version", headers.get("x-tarifia-client-version")),
                ("client_runtime", headers.get("x-tarifia-client-runtime")),
                ("client_update", headers.get("x-tarifia-client-update")),
            )
...
class PathRewriteMiddleware:
    ...
    async def send(self, message: Message, send: Send, replacements: int) -> None:
        ...
        if replacements > 0:
            headers["X-Tarifia-Deprecation-Notice"] = (
