"""Tests that mobile/web client identification (tarifia #12159) is attached to
PostHog *events*, and crucially NOT to person/profile properties.

Isolated from the main Tarifia infrastructure (see conftest.py): no database or
...
from tarifia.logging import ClientContext
...
def _run_request(scope: Scope, inside_request: Callable[[], None]) -> None:
    ...
    from tarifia.middlewares import LogCorrelationIdMiddleware
...
class TestPostHogClientContext:
    def test_client_identification_attached_to_events(
        ...
        _run_request(
            _http_scope(
                [
                    (b"x-tarifia-client-version", b"mobile/1.4.0"),
                    (b"x-tarifia-client-runtime", b"fingerprint-abc"),
                    (b"x-tarifia-client-update", b"update-xyz"),
                ]
            ),
    ...
    def test_client_identification_not_persisted_to_person(
        ...
        _run_request(
            _http_scope([(b"x-tarifia-client-version", b"mobile/1.4.0")]),
