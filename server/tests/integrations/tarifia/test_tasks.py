...
from tarifia.integrations.tarifia.tasks import (
    add_member,
    create_customer,
    delete_customer,
    remove_member,
...
def plain_service_mock(mocker: MockerFixture) -> MagicMock:
    ...
    mocker.patch("tarifia.integrations.tarifia.tasks.plain_service", mock)
    return mock
...
class TestAddMember:
    async def test_adds_member_to_existing_customer(
        ...
        client.add_member.assert_called_once_with(
            customer_id="tarifia-customer-123",
            email="user@example.com",
            name="User Example",
            external_id="user-123",
            role="member",
...
class TestFlushEventIngestion:
    ...
    def _patch_settings(self, mocker: MockerFixture, *, enabled: bool = True) -> None:
        settings = mocker.patch("tarifia.integrations.tarifia.tasks.settings")
        settings.TARIFIA_SELF_ENABLED = enabled
        settings.TARIFIA_ORGANIZATION_ID = str(self.SELF_ORG_ID)
    ...
    async def test_noop_when_no_counts(self, mocker: MockerFixture) -> None:
        ...
        repository.get_latest_tarifia_self_ingestion_timestamp = AsyncMock(
            return_value=None
        )
        mocker.patch(
            "tarifia.integrations.tarifia.tasks.EventRepository.from_session",
            return_value=repository,
        )
        mocker.patch(
            "tarifia.integrations.tarifia.tasks.count_user_events_by_organization",
            return_value={},
        )
        client = AsyncMock(spec=TarifiaSelfClient)
        mocker.patch("tarifia.integrations.tarifia.tasks.get_client", return_value=client)
