...
WEBHOOK_SECRET = "test_tarifia_webhook_secret"
WEBHOOK_URL = "/v1/integrations/tarifia/webhook"
...
def _set_webhook_secret(mocker: MockerFixture) -> None:
    mocker.patch(
        "tarifia.integrations.tarifia.endpoints.settings.TARIFIA_WEBHOOK_SECRET",
        WEBHOOK_SECRET,
    )
...
def enqueue_mock(mocker: MockerFixture) -> AsyncMock:
    return mocker.patch(
        "tarifia.integrations.tarifia.endpoints.external_event_service.enqueue",
        new=AsyncMock(),
    )
...
class TestWebhook:
    ...
    async def test_unhandled_event_type_is_ignored(
        ...
        # subscription.canceled is a valid (parseable) Tarifia event type, but
    ...
    async def test_implemented_event_is_enqueued(
        ...
        assert call.args[1].value == "tarifia"
        assert call.args[2] == f"tarifia_self.webhook.{event_type}"
    ...
    async def test_order_created_is_enqueued_with_delay(
        ...
        # generation have time to complete on Tarifia's side.
        ...
        call = enqueue_mock.await_args
        assert call is not None
        assert call.args[2] == "tarifia_self.webhook.order.created"
