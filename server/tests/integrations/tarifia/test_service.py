...
from tarifia.models.organization import Organization, SupportTier
...
def configured(mocker: MockerFixture) -> None:
    settings = mocker.patch("tarifia.integrations.tarifia.service.settings")
    settings.TARIFIA_SELF_ENABLED = True
    settings.TARIFIA_ORGANIZATION_ID = str(SELF_ORG_ID)
    settings.TARIFIA_FREE_PRODUCT_ID = "prod_free"
...
def organization_repository_mock(mocker: MockerFixture) -> MagicMock:
    ...
    mocker.patch(
        "tarifia.integrations.tarifia.service.OrganizationRepository.from_session",
...
def account_repository_mock(mocker: MockerFixture) -> MagicMock:
    ...
    mocker.patch(
        "tarifia.integrations.tarifia.service.AccountRepository.from_session",
        return_value=repository,
    )
    return repository
...
def set_platform_fee_mock(mocker: MockerFixture) -> AsyncMock:
    mock = AsyncMock()
    mocker.patch("tarifia.account.service.account.set_platform_fee", mock)
    return mock
...
def plain_update_tenant_tier_mock(mocker: MockerFixture) -> AsyncMock:
    mock = AsyncMock()
    mocker.patch(
        "tarifia.integrations.tarifia.service.plain_service.update_tenant_tier",
        mock,
    )
    return mock
...
def list_grants_mock(mocker: MockerFixture) -> AsyncMock:
    ...
    mocker.patch("tarifia.integrations.tarifia.service.get_client", return_value=client)
...
class TestEnqueueTrackOrganizationReviewUsage:
    def _call(
        ...
        *,
        ...
        tarifia_self.enqueue_track_organization_review_usage(
    ...
    def test_noop_when_not_configured(self, mocker: MockerFixture) -> None:
        settings = mocker.patch("tarifia.integrations.tarifia.service.settings")
        settings.TARIFIA_SELF_ENABLED = False
        enqueue = mocker.patch("tarifia.integrations.tarifia.service.enqueue_job")
    ...
    def test_enqueues_job_with_serialized_cost(
        ...
        enqueue.assert_called_once_with(
            "tarifia_self.track_organization_review_usage",
...
class TestResolveFreePlan:
    async def test_subscribed_org_gets_standard_free(
        ...
        plan = await tarifia_self.resolve_free_plan(
...
class TestResolveOrganizationId:
    ...
    def test_missing_external_id_raises(self) -> None:
        with pytest.raises(TarifiaSelfWebhookError, match="external_id"):
            tarifia_self._resolve_organization_id(_make_customer(external_id=None))

    def test_non_uuid_external_id_raises(self) -> None:
        with pytest.raises(TarifiaSelfWebhookError, match="not-a-uuid"):
            tarifia_self._resolve_organization_id(
...
class TestExtractTransactionFee:
    def test_valid(self) -> None:
        assert tarifia_self._extract_transaction_fee(
    ...
    def test_invalid_fee_percent_raises(self, metadata: dict[str, Any]) -> None:
        with pytest.raises(TransactionFeeBenefitError, match="fee_percent"):
            tarifia_self._extract_transaction_fee(metadata, _BENEFIT_ID)
...
class TestExtractSupport:
    def test_valid(self) -> None:
        assert tarifia_self._extract_support(
    ...
    def test_invalid_field_raises(self, metadata: dict[str, Any], match: str) -> None:
        with pytest.raises(SupportBenefitError, match=match):
            tarifia_self._extract_support(metadata, _BENEFIT_ID)
...
class TestApplyTransactionFee:
    async def test_active_grant_applies_fees(
        ...
        await tarifia_self._apply_transaction_fee(session_mock, ORG_A, grant)
    ...
    async def test_no_grant_resets_to_defaults(
        ...
        await tarifia_self._apply_transaction_fee(session_mock, ORG_A, None)
...
class TestApplySupport:
    async def test_active_grant(
        ...
        await tarifia_self._apply_support(session_mock, ORG_A, grant)
    ...
    async def test_no_grant_unsets_tier(
        ...
        await tarifia_self._apply_support(session_mock, ORG_A, None)
...
class TestHandleBenefitGrantEvent:
    async def test_created_with_transaction_fee_applies_current_state(
        ...
        await tarifia_self.handle_benefit_grant_event(session_mock, payload)
...
class TestApplyPreviewAccess:
    async def test_active_grant_enables_and_preserves_other_flags(
        ...
        await tarifia_self._apply_preview_access(
    ...
    async def test_no_grant_disables_flags(
        ...
        await tarifia_self._apply_preview_access(session_mock, ORG_A, None)
...
class TestCancelSubscription:
    async def test_not_configured_raises(self, mocker: MockerFixture) -> None:
        ...
        with pytest.raises(TarifiaSelfNotConfigured):
            await tarifia_self.cancel_subscription(organization_id=ORG_A)
    ...
    async def test_no_active_subscription_raises(
        ...
        with pytest.raises(TarifiaSelfNoActiveSubscription):
            await tarifia_self.cancel_subscription(organization_id=ORG_A)
...
class TestListPlans:
    ...
    async def test_sorts_by_metadata_order(
        ...
        plans = await tarifia_self.list_plans()
...
class TestGetSubscription:
    async def test_not_configured_raises(self, mocker: MockerFixture) -> None:
        ...
        with pytest.raises(TarifiaSelfNotConfigured):
            await tarifia_self.get_subscription(ORG_A)
    ...
    async def test_returns_none_when_no_active_sub(
        ...
        result = await tarifia_self.get_subscription(ORG_A)
...
class TestStartCheckout:
    async def test_not_configured_raises(
        ...
        with pytest.raises(TarifiaSelfNotConfigured):
            await tarifia_self.start_checkout(
    ...
    async def test_unknown_plan_raises(
        ...
        with pytest.raises(TarifiaSelfPlanNotFound):
            await tarifia_self.start_checkout(
    ...
    async def test_unapproved_rejected(
        ...
        with pytest.raises(TarifiaSelfNotApproved):
            await tarifia_self.start_checkout(
                session=read_session_mock,
                organization_id=ORG_A,
...
class TestChangePlan:
    async def test_not_configured_raises(
        ...
        with pytest.raises(TarifiaSelfNotConfigured):
            await tarifia_self.change_plan(
    ...
    async def test_clears_existing_discount_when_leaving_scale(
        ...
        mocker.patch(
            "tarifia.integrations.tarifia.service.settings.TARIFIA_SCALE_PRODUCT_ID",
            "prod_scale",
        )
        client_mock.list_recurring_products.return_value = [
    ...
    async def test_attaches_claimable_discount_before_product_switch(
        ...
        mocker.patch(
            "tarifia.integrations.tarifia.service.startup_program_service."
...
class TestClaimStartupProgram:
    ...
    async def test_not_configured_raises(
        ...
        with pytest.raises(TarifiaSelfNotConfigured):
            await tarifia_self.claim_startup_program(
    ...
    async def test_free_org_returns_checkout(
        ...
        mocker.patch(
            "tarifia.integrations.tarifia.service.settings.STARTUP_PROGRAM_ENABLED",
            True,
        )
        mocker.patch(
            "tarifia.integrations.tarifia.service.settings.TARIFIA_SCALE_PRODUCT_ID",
        ...
        subscription, checkout = await tarifia_self.claim_startup_program(
            session=read_session_mock,
            organization_id=ORG_A,
    ...
    async def test_raises_when_not_invited(
        ...
        from tarifia.startup_program.service import StartupProgramError
...
class TestListOrders:
    async def test_not_configured_raises(self, mocker: MockerFixture) -> None:
        ...
        with pytest.raises(TarifiaSelfNotConfigured):
            await tarifia_self.list_orders(ORG_A)
    ...
    async def test_returns_empty_when_customer_not_found(
        ...
        items, total = await tarifia_self.list_orders(ORG_A)
    ...
    async def test_lists_orders_for_resolved_customer(
        ...
        items, total = await tarifia_self.list_orders(ORG_A, page=2, limit=20)
...
class TestGetOrderInvoiceUrl:
    async def test_not_configured_raises(self, mocker: MockerFixture) -> None:
        ...
        with pytest.raises(TarifiaSelfNotConfigured):
            await tarifia_self.get_order_invoice_url(ORG_A, "ord_1")
    ...
    async def test_customer_not_found_raises_order_not_found(
        ...
        with pytest.raises(TarifiaSelfOrderNotFound):
            await tarifia_self.get_order_invoice_url(ORG_A, "ord_1")
    ...
    async def test_returns_invoice_url(
        ...
        url = await tarifia_self.get_order_invoice_url(ORG_A, "ord_1")
...
def enqueue_email_mock(mocker: MockerFixture) -> MagicMock:
    return mocker.patch("tarifia.integrations.tarifia.service.enqueue_email_template")
...
class TestHandleOrderCreatedEvent:
    async def test_ignores_non_subscription_billing_reasons(
        ...
        await tarifia_self.handle_order_created_event(payload)
    ...
    async def test_triggers_generation_and_retries_when_not_yet_generated(
        ...
        with pytest.raises(TarifiaSelfInvoiceNotReady):
            await tarifia_self.handle_order_created_event(payload)
