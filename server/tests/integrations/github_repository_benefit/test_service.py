...
from tarifia.integrations.github_repository_benefit.service import (
    github_repository_benefit_user_service,
)
from tarifia.integrations.github_repository_benefit.types import SimpleUser
from tarifia.models import OAuthAccount
from tarifia.models.user import OAuthPlatform
...
class TestGetBillingPlan:
    async def test_github_exception_logged_with_context(
        ...
        mocker.patch(
            "tarifia.integrations.github.client.get_app_installation_client",
            return_value=mock_client,
        )

        # Mock the logger to verify it's called with proper context
        mock_log = mocker.patch(
            "tarifia.integrations.github_repository_benefit.service.log"
        )
