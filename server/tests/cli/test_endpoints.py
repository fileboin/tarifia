...
from tarifia.cli.listener import LISTENER_KEY_PREFIX, has_active_listener
...
def mock_organization_service(mocker: MockerFixture, org_id: uuid.UUID) -> MagicMock:
    ...
    mock_org.id = org_id
    return mocker.patch(
        "tarifia.cli.endpoints.organization_service.get",
...
def mock_subscribe(mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch(
        "tarifia.cli.endpoints.subscribe",
...
class TestListenEndpoint:
    async def test_mark_active_on_connect(
        ...
        from tarifia.cli.endpoints import listen
