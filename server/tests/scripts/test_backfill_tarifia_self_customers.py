...
async def _setup_self_org(
    ...
    monkeypatch.setattr(settings, "TARIFIA_ORGANIZATION_ID", str(self_org.id))
    monkeypatch.setattr(settings, "TARIFIA_FREE_PRODUCT_ID", str(free_product.id))
    return self_org


@pytest.mark.asyncio
class TestBackfillTarifiaSelfCustomers:
