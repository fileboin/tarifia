...
from tarifia.email.sender import SendEmailError
from tarifia.email.tasks import email_send
from tarifia.enums import EmailSender
from tarifia.models.email_log import EmailLog, EmailLogStatus
...
class TestEmailSend:
    async def test_successful_send(
        ...
        mock_send = mocker.patch(
            "tarifia.email.tasks.email_sender.send",
        ...
        await email_send(
            ...
            from_name="Tarifia",
            from_email_addr="noreply@tarifia.sh",
            email_headers=None,
            reply_to_name=None,
    ...
    async def test_template_email_extracts_organization_id(
        ...
        mocker.patch(
            "tarifia.email.tasks.render_from_json",
    ...
    async def test_processor_reflects_settings(
        ...
        mocker.patch(
            "tarifia.email.tasks.settings.EMAIL_SENDER",
            EmailSender.logger,
        )
    ...
    async def test_log_failure_does_not_mask_send_error(
        ...
        mocker.patch(
            "tarifia.email.tasks.AsyncSessionMaker",
            side_effect=RuntimeError("db down"),
        )
        log_exception = mocker.patch("tarifia.email.tasks.log.exception")
