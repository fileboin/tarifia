...
from tarifia.enums import RateLimitGroup
from tarifia.rate_limit import (
    _PRODUCTION_RULES,
    _SANDBOX_RULES,
    _authenticate,
    _bearer_token,
...
class TestBearerToken:
    def test_returns_token(self) -> None:
        scope = _http_scope(headers=[(b"authorization", b"Bearer tarifia_pat_xyz")])
        assert _bearer_token(scope) == "tarifia_pat_xyz"
...
class TestSessionCookie:
    def test_returns_value(self) -> None:
        header = f"{settings.USER_SESSION_COOKIE_KEY}=tarifia_us_xyz".encode("ascii")
        scope = _http_scope(headers=[(b"cookie", header)])
        assert _session_cookie(scope) == "tarifia_us_xyz"

    def test_among_other_cookies(self) -> None:
        header = (
            f"foo=bar; {settings.USER_SESSION_COOKIE_KEY}=tarifia_us_xyz; baz=qux"
        ).encode("ascii")
        scope = _http_scope(headers=[(b"cookie", header)])
        assert _session_cookie(scope) == "tarifia_us_xyz"
...
class TestAuthenticate:
    async def test_cache_miss_uses_token_hash_pending_auth(self, redis: Redis) -> None:
        identity = await _authenticate(
            _http_scope(
                headers=[(b"authorization", b"Bearer tarifia_pat_unknown")],
                client=("8.8.8.8", 1234),
            ),
            redis=redis,
        )
        assert identity == (
            f"token:{_token_hash('tarifia_pat_unknown')}",
    ...
    async def test_cookie_cache_miss_uses_cookie_hash_pending_auth(
        ...
        assert identity == (
            f"cookie:{_token_hash('tarifia_us_unknown')}",
            RateLimitGroup.pending_auth,
        )

    async def test_bearer_token_preferred_over_cookie(self, redis: Redis) -> None:
        await write_cached_identity(
            redis, "tarifia_pat_a", ("user:bearer", RateLimitGroup.elevated)
        )
        await write_cached_identity(
        ...
        identity = await _authenticate(
            _http_scope(
                headers=[
                    (b"authorization", b"Bearer tarifia_pat_a"),
    ...
    async def test_distinct_tokens_get_distinct_identities(self, redis: Redis) -> None:
        token_a = "tarifia_pat_a"
        token_b = "tarifia_pat_b"
        ...
        ident_a = await _authenticate(
            _http_scope(headers=[(b"authorization", b"Bearer tarifia_pat_a")]),
            redis=redis,
        )
        ident_b = await _authenticate(
            _http_scope(headers=[(b"authorization", b"Bearer tarifia_pat_b")]),
            redis=redis,
        )
...
class TestSensitiveEndpointZoneIsolation:
    ...
    Regression coverage for https://github.com/tarifiasource/tarifia/issues/11704.
    """
