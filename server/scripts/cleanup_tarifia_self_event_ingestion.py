...
async def run(
    ...
    """Delete all `event_ingestion` events from the Tarifia-for-Tarifia org.

    These are the per-org rollup events emitted by the (now disabled)
    `tarifia_self.track_event_ingestion_v2` cron. Postgres deletes in
    ...
    if not settings.TARIFIA_ORGANIZATION_ID:
        typer.echo("TARIFIA_ORGANIZATION_ID is not configured.", err=True)
