"""Seed transaction-fee benefits and tier products via the Tarifia SDK.
...
Usage:
    ...
    TARIFIA_ACCESS_TOKEN=... uv run python -m scripts.seed_tarifia_for_tarifia --env local

    # Sandbox
    TARIFIA_ACCESS_TOKEN=... uv run python -m scripts.seed_tarifia_for_tarifia --env sandbox

    # Production
    TARIFIA_ACCESS_TOKEN=... uv run python -m scripts.seed_tarifia_for_tarifia --env prod
...
Tarifia self-billing plan catalog and are also imported by the dev seed loader
...
def _make_client(env: str, token: str) -> Tarifia:
    if env == "local":
        return Tarifia(access_token=token, server_url=LOCAL_SERVER_URL)
    if env == "sandbox":
        return Tarifia(access_token=token, server="sandbox")
    if env == "prod":
        return Tarifia(access_token=token, server="production")
    raise typer.BadParameter(f"Unknown env: {env}")


async def _existing_benefits(
    tarifia: Tarifia, organization_id: str | None
) -> dict[str, Benefit]:
...
async def _seed_benefits(
    tarifia: Tarifia, organization_id: str | None, dry_run: bool
) -> dict[str, Benefit]:
    typer.echo("Listing existing benefits...")
    existing = await _existing_benefits(tarifia, organization_id)
    ...
    for benefit in BENEFITS:
        ...
        if current is None:
            ...
            created = await tarifia.benefits.create_async(request=request)
        ...
        await tarifia.benefits.update_async(id=current.id, request_body=update)
        typer.echo(
...
async def _seed_products(
    ...
    existing = await _existing_products(tarifia, organization_id)
    ...
    for product in PRODUCTS:
        ...
        if current is None:
            ...
            created = await tarifia.products.create_async(request=request)
            await tarifia.products.update_benefits_async(
                id=created.id,
        ...
        if metadata_changed or visibility_changed or description_changed:
            if dry_run:
                ...
                await tarifia.products.update_async(
                    id=current.id,
...
async def _seed_subscription_settings(
    ...
    if organization_id is None:
        ...
        return
    org = await tarifia.organizations.get_async(id=organization_id)
    ...
    await tarifia.organizations.update_async(
        id=organization_id,
...
async def _seed(
    ...
    tarifia = _make_client(env, token)
    mode = " (dry-run, no changes will be made)" if dry_run else ""
    typer.echo(f"Seeding on {env}{mode}...")
    benefits_by_description = await _seed_benefits(tarifia, organization_id, dry_run)
    if seed_products:
        await _seed_products(tarifia, organization_id, benefits_by_description, dry_run)
    else:
        typer.echo("Skipping products (pass --products to seed).")
    await _seed_subscription_settings(tarifia, organization_id, dry_run)
...
def main(
    ...
    token: Annotated[
        ...
        typer.Option(
            "--token",
            "-t",
            help="Tarifia access token. Falls back to $TARIFIA_ACCESS_TOKEN.",
        ),
    ] = None,
    ...
    access_token = token or os.environ.get("TARIFIA_ACCESS_TOKEN")
    if not access_token:
        raise typer.BadParameter(
            "Missing access token: pass --token or set TARIFIA_ACCESS_TOKEN."
        )
