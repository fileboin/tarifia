...
from tarifia.customer.schemas.customer import CustomerTeamCreate
...
from tarifia.subscription.schemas import SubscriptionCreateExternalCustomer
...
async def run_backfill(
    *,
    ...
    self_org = await session.get(
        Organization, uuid.UUID(settings.TARIFIA_ORGANIZATION_ID)
    )
    if self_org is None:
        raise RuntimeError(
    ...
    typer.echo("Loading existing Tarifia self customers...")
...
async def backfill(
    ...
    """Backfill Tarifia customers, members, and subscriptions for all active organizations."""
