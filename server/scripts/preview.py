...
DEFAULT_PREVIEW_BASE_DOMAIN = "preview.tarifia.sh"
...
def get_preview_base_domain() -> str:
    return (
        os.getenv("TARIFIA_PREVIEW_BASE_DOMAIN") or DEFAULT_PREVIEW_BASE_DOMAIN
    ).strip()
...
def load_preview_postgres_admin_config() -> PreviewPostgresAdminConfig:
    admin_dsn = os.getenv("TARIFIA_PREVIEW_POSTGRES_ADMIN_DSN")
    if not admin_dsn:
        raise RuntimeError("TARIFIA_PREVIEW_POSTGRES_ADMIN_DSN is required")

    parsed_admin_dsn = urlparse(admin_dsn)
    if not parsed_admin_dsn.scheme or parsed_admin_dsn.hostname is None:
        raise RuntimeError(
            "TARIFIA_PREVIEW_POSTGRES_ADMIN_DSN must be a valid Postgres DSN"
        )

    app_host_override = os.getenv("TARIFIA_PREVIEW_POSTGRES_APP_HOST")
    ...
    app_port_raw = (os.getenv("TARIFIA_PREVIEW_POSTGRES_APP_PORT") or "").strip()
    if not app_port_raw:
    ...
    template_database_raw = (
        os.getenv("TARIFIA_PREVIEW_POSTGRES_TEMPLATE_DATABASE") or ""
    ).strip()
    template_database: str | None
    if template_database_raw:
        template_database = validate_safe_postgres_identifier(
            template_database_raw,
            env_name="TARIFIA_PREVIEW_POSTGRES_TEMPLATE_DATABASE",
        )
    else:
...
def load_preview_tinybird_admin_config() -> PreviewTinybirdAdminConfig:
    api_url = (os.getenv("TARIFIA_PREVIEW_TINYBIRD_API_URL") or "").strip()
    if not api_url:
        raise RuntimeError("TARIFIA_PREVIEW_TINYBIRD_API_URL is required")

    admin_token = (os.getenv("TARIFIA_PREVIEW_TINYBIRD_ADMIN_TOKEN") or "").strip()
    if not admin_token:
        raise RuntimeError("TARIFIA_PREVIEW_TINYBIRD_ADMIN_TOKEN is required")

    workspace_name = (os.getenv("TARIFIA_PREVIEW_TINYBIRD_WORKSPACE") or "").strip()
    if not workspace_name:
        raise RuntimeError("TARIFIA_PREVIEW_TINYBIRD_WORKSPACE is required")
    ...
    config = PreviewTinybirdAdminConfig(
        ...
        last_partition=parse_env_bool(
            os.getenv("TARIFIA_PREVIEW_TINYBIRD_LAST_PARTITION")
        ),
    )
    log_preview(
...
def build_preview_postgres_env(
    *,
    ...
    return {
        "TARIFIA_POSTGRES_DATABASE": database_name,
        "TARIFIA_POSTGRES_HOST": config.app_host,
        "TARIFIA_POSTGRES_PORT": str(config.app_port),
        "TARIFIA_POSTGRES_PWD": password,
        "TARIFIA_POSTGRES_USER": role_name,
    }
