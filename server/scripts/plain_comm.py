    ...
    async with _get_plain_client() as plain:
        ...
        thread_result = await plain.create_thread(
            pl.CreateThreadInput(
                ...
                title="Tarifia Incident: Incorrect webhook events (Jan 19–21)",
...
I’m Birk, Founder & CEO of Tarifia.
...
Importantly: **No customer-facing emails** about these state changes were sent by Tarifia.
...
I sincerely apologize for the impact this incident may have had on you. We understand that Tarifia is a critical part of your infrastructure, and incidents like this can damage trust. We take that responsibility seriously and are implementing multiple improvements to prevent this from happening again.
...
"""
        ...
        email_result = await plain.send_new_email(
            pl.SendNewEmailInput(
                customer_id=customer_id,
                thread_id=thread_id,
                subject="Tarifia Incident: Incorrect webhook events (Jan 19–21)",
