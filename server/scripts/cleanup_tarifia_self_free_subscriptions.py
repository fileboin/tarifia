...
from tarifia.models import Event, Order, Subscription
...
async def cleanup(
    ...
    """Delete free-plan Subscriptions and their $0 Orders for the Tarifia org.

    Tarifia-for-Tarifia originally modeled the free tier as a real Subscription on
    TARIFIA_FREE_PRODUCT_ID. We're rolling that decision back so the free plan is
    ...
    Pause the dramatiq worker (or at least the subscription.cycle / tarifia_self
    ...
    organization_id = uuid.UUID(settings.TARIFIA_ORGANIZATION_ID)
    free_product_id = uuid.UUID(settings.TARIFIA_FREE_PRODUCT_ID)
