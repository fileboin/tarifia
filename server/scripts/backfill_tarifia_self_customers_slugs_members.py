...
from tarifia.models import Customer, Organization, User
from tarifia.models.member import Member
...
async def run_backfill(
    *,
    ...
    typer.echo("Loading Tarifia self customers...")
...
async def backfill(
    ...
    """Backfill organization slug into Tarifia self customers' user_metadata,
    and repair member names from the linked User."""
    configure_script_logging()

    if not settings.TARIFIA_ORGANIZATION_ID:
        typer.echo("TARIFIA_ORGANIZATION_ID is not configured, aborting.")
        raise typer.Exit(1)
