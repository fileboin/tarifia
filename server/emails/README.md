...
1. On Python's side, in `server/tarifia/email/schemas.py`, add a new item to `EmailTemplate` and implement a Pydantic schema to describe the props of the email.

```python
# server/tarifia/email/schemas.py
...
```python
from tarifia.email.schemas import CustomerGreetingsEmail, CustomerGreetingsProps
