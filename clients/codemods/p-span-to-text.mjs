#!/usr/bin/env node
/**
 * Codemod: replace <p> and <span> with <Text /> from @tarifia-sh/orbit
 *
 * Usage:
 ...
 *   </span>        →  </Text>
 *
 * The import { Text } from '@tarifia-sh/orbit' is added/updated automatically.
 *
...
/**
 * Ensures `Text` is imported from `@tarifia-sh/orbit` in the given source string.
 * - If an import from `@tarifia-sh/orbit` already exists and includes `Text` → no-op
 * - If an import from `@tarifia-sh/orbit` exists without `Text` → adds `Text` to it
...
function ensureTextImport(source, filePath) {
  ...
  for (const stmt of sourceFile.statements) {
    if (
      ...
      stmt.moduleSpecifier.text === '@tarifia-sh/orbit'
    ) {
  ...
  // No @tarifia-sh/orbit import — add one.
  ...
  const importLine = `import { Text } from '@tarifia-sh/orbit'\n`
