import { getCurrencyDecimalFactor, isDecimalCurrency } from '@tarifia-sh/currency'
...
const MoneyInput = (props: Props) => {
  ...
  const currencyLabel = (
    <span className="dark:text-tarifia-500 text-sm font-medium text-gray-500">
  ...
  return (
    <Input
      ...
      name={name}
      className={twMerge(
        'dark:placeholder:text-tarifia-500 block w-full px-4 pl-14 text-base font-normal placeholder:text-gray-400',
        props.className ?? '',
      )}
      value={internalValue}
      onChange={onChange}
