      ...
      >
        ...
        >
          <motion.div
            ...
            className={twMerge(
              'dark:bg-tarifia-900 relative z-10 flex h-full max-h-full w-full flex-col overflow-y-auto bg-white shadow-sm md:fixed md:top-0 md:right-0 md:bottom-0 md:h-auto md:w-[540px] dark:text-white',
              className,
            )}
...
const CloseButton = (props: { className?: string; hide: () => void }) => {
  return (
    <button
      type="button"
      className={twMerge(
        'dark:text-tarifia-600 dark:hover:text-tarifia-500/70 cursor-pointer text-gray-300 hover:text-gray-400',
        props.className,
      )}
      onClick={() => props.hide()}
      tabIndex={-1}
