...
const PREVIEW_BUILD = process.env.TARIFIA_PREVIEW_BUILD === '1'

// Vercel preview: compute basePath and API URL from PR number + Tailscale hostname
let previewBasePath = ''
if (
  process.env.VERCEL_GIT_PULL_REQUEST_ID &&
  process.env.TARIFIA_PREVIEW_BACKEND_HOST
) {
  const prNum = parseInt(process.env.VERCEL_GIT_PULL_REQUEST_ID)
  previewBasePath = `/pr-${prNum}`
  const baseUrl = `https://${process.env.TARIFIA_PREVIEW_BACKEND_HOST}${previewBasePath}`
...
const nonEmbeddedCSP = `
  ${baseCSP}
  form-action 'self' ${process.env.NEXT_PUBLIC_API_URL} tarifia:;
...
// We rewrite Mintlify docs to tarifia.sh/docs, so we need a specific CSP for them
...
const docsCSP = `
  ...
  frame-src 'self' *.mintlify.dev https://tarifia-public-assets.s3.us-east-2.amazonaws.com;
`
...
const nextConfig = {
  ...
  transpilePackages: ['shiki', '@tarifia-sh/checkout', '@tarifia-sh/orbit'],
  pageExtensions: ['js', 'jsx', 'md', 'mdx', 'ts', 'tsx'],

  ...(previewBasePath && {
    basePath: previewBasePath,
    env: {
      TARIFIA_API_URL: `https://${process.env.TARIFIA_PREVIEW_BACKEND_HOST}:8443${previewBasePath}`,
    },
  }),
  ...
  async rewrites() {
    const apiUrl = process.env.TARIFIA_API_URL || process.env.NEXT_PUBLIC_API_URL
    return [
  ...
  async redirects() {
    return [
      // dashboard.tarifia.sh redirections
      {
        source: '/',
        destination: '/auth',
        has: [
      ...
      {
        source: '/:path*',
        destination: 'https://tarifia.sh/:path*',
        has: [
          {
            type: 'host',
            value: 'dashboard.tarifia.sh',
      ...
      {
        source: '/careers',
        destination: 'https://tarifia.sh/company',
        permanent: false,
      },
      {
        source: '/legal/terms',
        destination: 'https://tarifia.sh/legal/master-services-terms',
        permanent: false,
      },
      {
        source: '/legal/privacy',
        destination: 'https://tarifia.sh/legal/privacy-policy',
        permanent: false,
      },
      {
        source: '/llms.txt',
        destination: 'https://tarifia.sh/docs/llms.txt',
        permanent: true,
        has: [
          {
            type: 'host',
      ...
      },
      {
        ...
        destination: 'https://tarifia.sh/docs/llms-full.txt',
        permanent: true,
        has: [
          {
            type: 'host',
      ...
      // Old blog redirects
      {
        source: '/tarifiasource/posts',
        destination: '/blog',
        permanent: false,
      },
      {
        source: '/tarifiasource/posts/:path(.*)',
      ...
      {
        source: '/:path*',
        destination: 'https://tarifia.sh/tarifiasource',
        has: [
          {
            type: 'host',
            value: 'blog.tarifia.sh',
          },
        ],
        permanent: false,
      },
      ...
      // CLI Install Script
      {
        source: '/install.sh',
        destination:
          'https://raw.githubusercontent.com/tarifiasource/cli/main/install.sh',
        permanent: false,
      },
