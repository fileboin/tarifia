...
export const getSSRHeaders = (): Record<string, string> => {
  ...
  if (process.env.TARIFIA_PREVIEW_ACCESS_TOKEN) {
    headers['X-Preview-Token'] = process.env.TARIFIA_PREVIEW_ACCESS_TOKEN
  }
...
export const createServerSideAPI = async (
  ...
  if (process.env.TARIFIA_PREVIEW_ACCESS_TOKEN) {
    apiHeaders = {
      ...apiHeaders,
      'X-Preview-Token': process.env.TARIFIA_PREVIEW_ACCESS_TOKEN,
    }
  }

  // Use TARIFIA_API_URL for server-side requests (e.g., in Docker containers)
  // Fall back to NEXT_PUBLIC_API_URL for local development
  const apiUrl =
    process.env.TARIFIA_API_URL || (process.env.NEXT_PUBLIC_API_URL as string)
