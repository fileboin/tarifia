...
const S3_URL =
  'https://tarifia-public-files.s3.amazonaws.com/product_media/abc/image.png'
...
describe('getResizedImage', () => {
  ...
  it('should not resize local Minio URLs', () => {
    const localUrl = 'http://127.0.0.1:9000/tarifia-s3-public/media/image.png'
  ...
  it('should snap to exact size when retina width matches', () => {
    expect(getResizedImage(S3_URL, 32)).toBe(
      'https://uploads.tarifia.sh/product_media/abc/image.png?width=64',
    )
  })
