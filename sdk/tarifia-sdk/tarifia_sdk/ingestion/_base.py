...
if TYPE_CHECKING:
    from tarifia_sdk import EventMetadataInput, EventsModelTypedDict

    from .strategies._base import S

logger = logging.getLogger("tarifia_sdk.ingestion")


class Ingestion:
    """
    Event ingestion client for Tarifia.

    This class handles the ingestion of events into the Tarifia API without blocking
    ...
    def __init__(
        ...
        *,
        ...
        self._client = self._stack.enter_context(
            Tarifia(access_token=access_token, server=server, server_url=server_url)
        )
