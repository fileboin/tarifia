...
class TarifiaError(Exception):
    def __init__(self, message: str):
        self.message = message
        super().__init__(message)


class TarifiaNetworkError(TarifiaError):
    def __init__(self, message: str):
        super().__init__(f"Tarifia API network error: {message}")


class TarifiaServerError(TarifiaError):
    def __init__(self, status_code: int, message: str):
        self.status_code = status_code
        super().__init__(
            f"Tarifia API returned a server error: {status_code} - {message}"
        )


class TarifiaClientError(TarifiaError):
    ...
    def __init__(self, status_code: int, error: typing.Any):
        self.status_code = status_code
        self.error = error
        super().__init__(f"Tarifia API returned an error: {status_code} - {error}")
...
class SyncClientBase(BuildRequestMixin):
    ...
    def send_request(self, request: httpx.Request) -> httpx.Response:
        try:
            ...
            raise TarifiaNetworkError(str(e)) from e
...
E = typing.TypeVar("E", bound=TarifiaClientError)
...
def _handle_errors(
    ...
    if response.is_server_error:
        raise TarifiaServerError(status_code, response.text)
    ...
    if response.is_client_error:
        try:
            ...
            raise TarifiaClientError(status_code, response.text)
