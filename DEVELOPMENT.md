...
Tarifia's stack consists of the following elements:
...
For the Tarifia stack to run properly, it needs quite a bunch of settings defined as environment variables. To ease things, we provide a script to bootstrap them. It requires [uv](https://docs.astral.sh/uv/getting-started/installation/) to be installed on your system.
...
If you work with multiple Git worktrees, secrets (GitHub, Stripe) are automatically shared via `~/.config/tarifia/secrets.env`:
...
3. For Stripe, edit `~/.config/tarifia/secrets.env` and add your keys (see template at `dev/secrets.env.template`)
4. Run `./dev/setup-environment` in each additional worktree - secrets are merged automatically

You can override the secrets file location with `TARIFIA_SECRETS_FILE` environment variable.
...
> [!IMPORTANT]
> Put all Stripe values in the central secrets file `~/.config/tarifia/secrets.env`, **not** in `server/.env`.
...
> writes that one value to *both* `TARIFIA_STRIPE_WEBHOOK_SECRET` and `TARIFIA_STRIPE_CONNECT_WEBHOOK_SECRET`,
...
2. **Copy your API keys** from the [Stripe API Keys page](https://dashboard.stripe.com/test/apikeys) and add them to your `~/.config/tarifia/secrets.env` file:

    ```
    TARIFIA_STRIPE_SECRET_KEY=sk_test_...
    TARIFIA_STRIPE_PUBLISHABLE_KEY=pk_test_...
    ```
...
4. **Create webhook endpoints** to handle Stripe events:
    ...
    - Set enabled events to only the events listed in `DIRECT_IMPLEMENTED_WEBHOOKS` (see `tarifia/integrations/stripe/endpoints.py`)
    ...
    - Copy the webhook signing secret and add it to your `~/.config/tarifia/secrets.env` file:
        ```
        TARIFIA_STRIPE_WEBHOOK_SECRET=whsec_...
        ```
    - Restart the same operation with:
        - events listed in `CONNECT_IMPLEMENTED_WEBHOOKS` (see `tarifia/integrations/stripe/endpoints.py`)
        - Set the endpoint URL to: `https://your-domain.ngrok-free.app/v1/integrations/stripe/webhook-connect`
        - Copy the webhook signing secret and add it to your `~/.config/tarifia/secrets.env` file:
            ```
            TARIFIA_STRIPE_CONNECT_WEBHOOK_SECRET=whsec_...
            ```
...
4. To use the seeded admin, use **admin@tarifia.sh** ([seeds_load.py](server/scripts/seeds_load.py)).
...
4. **Update `server/.env`** to allow CORS from ngrok and set the frontend URL:
    ...
    ```
    TARIFIA_CORS_ORIGINS='["http://localhost:3000", "http://127.0.0.1:3000", "https://github.com", "https://your-ngrok-url.ngrok-free.app"]'
    TARIFIA_FRONTEND_BASE_URL="https://your-ngrok-url.ngrok-free.app"
    ```
