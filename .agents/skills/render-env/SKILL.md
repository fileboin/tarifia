...
Do **not** ask about `lifecycle { ignore_changes = [value] }`. New variables here have no `value` baked into the Terraform code, so there's nothing for Terraform to overwrite — TFC just holds whatever's typed into the UI, and `ignore_changes` would be a no-op. The handful of existing blocks that include it (e.g. `tarifia_organization_id`, `customer_portal_url_overrides`) seed a default `value` in code *and* want UI overrides to stick; that's a different shape from a fresh secret and the user will tell you up-front if they want it.

## Naming convention

Follow the **modern bare-key** pattern that recent additions use (e.g. `tarifia_access_token`, `tinybird_api_token`, `customer_portal_url_overrides`):
...
- That the variable is now declared but **not yet consumed**. To consume it, they need to:
  ...
  2. **Wire the field into the matching `render_env_group` block in `terraform/modules/render_service/main.tf`** as `TARIFIA_${NAME_UPPER} = { value = var.<object>.<field> }`. Two backend groups exist:
     - `render_env_group "backend"` — applied to **every** environment.
     - `render_env_group "backend_production"` — production-only values (e.g. `TARIFIA_BACKOFFICE_HOST`, `TARIFIA_PLAIN_TOKEN`). Put a var here when sandbox/test should not see it.
  ...
  5. **If this is a `TARIFIA_*` env var, also add the field to the `Settings` class in `server/tarifia/config.py`** (Pydantic `BaseSettings` with `env_prefix="tarifia_"`; the env var name is `TARIFIA_<FIELD_NAME>`).
