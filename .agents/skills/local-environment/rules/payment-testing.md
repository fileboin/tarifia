...
```bash
docker logs --since 30s tarifia-app-<N>-api-1 2>&1 | grep -A1 "LOGIN CODE"
```

Use `admin@tarifia.sh` as the default test account — the seed creates it with
...
Stripe CLI if missing, logs in, writes `TARIFIA_STRIPE_SECRET_KEY`,
`TARIFIA_STRIPE_PUBLISHABLE_KEY`, and `TARIFIA_STRIPE_WEBHOOK_SECRET` into the
...
```bash
docker exec tarifia-app-<N>-api-1 sh -c 'cd /app/server && uv run python -c "
import asyncio, dramatiq
import tarifia.tasks  # registers every actor as a side-effect of import
...
- `import tarifia.tasks` is required. Without it, the broker has no registered
...
- The `JobQueueManager.open(...)` context manager is what flushes the queued
  ...
  `tarifia.job_queue_manager` context var.
...
```bash
dev docker exec db psql -U tarifia -d tarifia_dev_<N> -c \
