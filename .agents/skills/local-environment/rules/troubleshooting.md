...
```bash
# Each instance has its own database: tarifia_dev_<instance-number>
dev docker exec db psql -U tarifia -d tarifia_dev_<N> -c "SELECT 1"
```

Shared infra (db/redis/minio) is on the `tarifia-shared` Docker network with no
host port — reach it through `dev docker exec` or `docker exec tarifia-shared-<svc>-1`.
...
same time and OOM. Symptom in `docker logs tarifia-app-<N>-api-1`:
...
```bash
docker start tarifia-app-<N>-api-1 tarifia-app-<N>-web-1
```
...
If `tarifia-shared-db-1` was recreated (e.g. you ran `dev docker down` on the
...
```bash
docker restart tarifia-app-<N>-api-1 tarifia-app-<N>-worker-1
```

## Don't Mix `dev docker` and Bare `docker compose`

`dev docker` runs the shared infra under the project name `tarifia-shared` on
the `tarifia-shared` network. Running `cd server && docker compose up` from the
