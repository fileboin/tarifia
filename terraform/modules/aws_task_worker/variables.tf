...
variable "name" {
  description = "Short task name used in the Lambda function name: tarifia-{environment}-worker-{name}."
  type        = string
  ...
  validation {
    condition     = length("tarifia-production-worker-${var.name}") <= 64
    error_message = "Name is too long. tarifia-production-worker-{name} must be 64 characters or fewer."
  }
}
