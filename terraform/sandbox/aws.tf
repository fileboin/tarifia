data "aws_iam_policy" "permission_boundary" {
  name = "TarifiaPermissionBoundary"
}

module "lambda_worker_ecr" {
  source = "../modules/ecr_repository"

  name = "tarifia-sandbox-lambda-worker"
}

module "redis" {
  source = "../modules/aws_redis"

  name       = "tarifia-sandbox-worker"
...
module "dummy_lambda_worker" {
  ...
  environment              = "sandbox"
  name                     = "dummy"
  queue_name               = "tarifia-sandbox-tasks-dummy"
  ...
  environment_variables = {
    TARIFIA_ENV                     = "sandbox"
    TARIFIA_BASE_URL                = "https://sandbox-api.tarifia.sh"
    TARIFIA_FRONTEND_BASE_URL       = "https://sandbox.tarifia.sh"
    TARIFIA_CHECKOUT_BASE_URL       = "https://sandbox-api.tarifia.sh/v1/checkout-links/{client_secret}/redirect"
    TARIFIA_JWKS                    = "/tmp/jwks.json"
    TARIFIA_LOG_LEVEL               = "INFO"
    TARIFIA_TESTING                 = "0"
    TARIFIA_POSTGRES_DATABASE       = "tarifia_sandbox"
    TARIFIA_POSTGRES_HOST           = local.db_external_host
    TARIFIA_POSTGRES_PORT           = local.db_port
    TARIFIA_POSTGRES_USER           = local.db_user
    TARIFIA_POSTGRES_SSL            = "true"
    TARIFIA_REDIS_HOST              = module.redis.host
    TARIFIA_REDIS_PORT              = tostring(module.redis.port)
    TARIFIA_REDIS_DB                = "1"
    TARIFIA_AWS_REGION              = "us-east-2"
    TARIFIA_WORKER_SQS_ENABLED      = "true"
    TARIFIA_WORKER_SQS_QUEUE_PREFIX = "tarifia-sandbox-tasks"
  }

  secret_environment_variables = {
    TARIFIA_CURRENT_JWK_KID = var.backend_current_jwk_kid_sandbox
    TARIFIA_JWKS_CONTENT    = var.backend_jwks_sandbox
    TARIFIA_LOGFIRE_TOKEN   = var.logfire_token
    TARIFIA_POSTGRES_PWD    = local.db_password
    TARIFIA_SECRET          = var.backend_secret_sandbox
    TARIFIA_SENTRY_DSN      = var.backend_sentry_dsn_sandbox
...
resource "aws_iam_user" "tasks_producer" {
  name = "tarifia-sandbox-tasks-producer"
}
